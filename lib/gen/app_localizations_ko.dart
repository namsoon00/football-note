...
class AppLocalizationsKo extends AppLocalizations {
  ...
  @override
  String get diaryStickerFortune => '플레이 힌트';
  ...
  @override
  String get diaryFortunePinSummary => '오늘 기록에 남은 플레이 힌트를 다이어리 스티커로 붙여둘 수 있어요.';

  @override
  String get diaryFortuneStorySentence =>
      '오늘 플레이 힌트에서 기억하고 싶은 장면이나 응원 한 줄을 적어 본다.';

  @override
  String get diaryFortuneNoteTitle => '오늘 플레이 힌트 메모';
  ...
  @override
  String get fortuneDialogTitle => '오늘의 플레이 힌트';

  @override
  String get fortuneDialogSubtitle => '생일과 이름으로 고른 오늘의 가벼운 플레이 힌트예요.';

  @override
  String get fortuneDialogOverviewTitle => '힌트 보기';

  @override
  String get fortuneDialogOverallFortuneLabel => '오늘 힌트';
  ...
  @override
  String get fortuneDialogPoolSizeLabel => '힌트 조합';
  ...
  @override
  String get fortuneDialogEncouragement => '오늘도 재미있는 한 장면을 만들어봐요.';

  @override
  String get fortuneDialogAction => '좋아요';

  @override
  String get entryFortuneOpenFailed => '플레이 힌트 화면을 여는 중 문제가 생겼어요.';
  ...
  String fortuneGeneratedLuckyInfoLine(
    ...
    return '재미 포인트: 숫자 $number, 색상 $color, 시간대 $time. $zone에서 $cue.';
  }
  ...
  String get fortuneSajuElementFlows =>
      '새로 배우는 힘|표현과 자신감|균형 잡기|정확도 올리기|회복과 관찰|앞으로 나가는 에너지|차분한 집중|방향 전환 감각|빠른 판단|부드러운 템포|넓은 시야|단단한 중심|가벼운 출발|마무리 감각|팀 호흡|실수 뒤 회복';

  @override
  String get fortuneSajuFortuneThemes =>
      '새 동작을 가볍게 시도하기|친구와 타이밍 맞추기|작은 실수도 빠르게 넘기기|주변을 더 넓게 보기|몸의 중심 다시 잡기|집중 포인트 하나 정하기|말보다 움직임으로 보여주기|차분하게 속도 올리기|첫 선택을 빠르게 정하기|한 번 쉬고 다시 도전하기|좋은 패스 길을 먼저 보기|압박을 재미있게 풀어보기|마지막 터치까지 산뜻하게 가져가기|친구의 움직임을 먼저 읽기|새로운 방향으로 몸을 틀어보기|짧은 성공을 계속 쌓기|공을 받기 전 미리 준비하기|수비 전환을 가볍게 시작하기|마무리 전에 호흡 맞추기|양발을 번갈아 써보기|공 없는 움직임으로 공간 만들기|오늘의 좋은 장면 하나 남기기|템포를 스스로 조절하기|칭찬 한마디로 분위기 살리기';

  @override
  String get fortuneSajuTrainingTones =>
      '첫 터치의 방향 정리|짧은 패스 후 다음 선택|몸을 열고 받는 움직임|수비 전환 첫 걸음|마무리 전 호흡 조절|양발 밸런스 확인|공 없는 움직임의 타이밍|리턴 패스 각도 만들기|고개 들고 받기|압박 전에 몸 돌리기|약한 발로 짧게 연결하기|볼을 멈춘 뒤 바로 보기|첫 세트는 정확하게|마지막 세트는 가볍게|터치 수 줄여보기|드리블 후 패스 연결|패스 후 빈 공간 찾기|수비 뒤로 빠지는 움직임|세컨드볼 반응 준비|턴 동작을 작게 만들기|스프린트 첫 세 걸음|슈팅 전 발 위치|코치 말 한 가지 기억하기|친구 이름 부르고 패스하기';

  @override
  String get fortuneSajuNameElements =>
      '빠른 출발형|밝게 표현하는 형|안정적으로 버티는 형|정확하게 맞추는 형|유연하게 바꾸는 형|자신 있게 치고 나가는 형|꾸준히 쌓아가는 형|잘 보고 움직이는 형|분위기를 살리는 형|한 박자 기다리는 형|도전을 좋아하는 형|팀을 연결하는 형|조용히 집중하는 형|마지막까지 버티는 형|새 길을 찾는 형|리듬을 만드는 형';

  @override
  String get fortuneSajuPlayAdvice =>
      '처음엔 가볍게 시작해도 마지막 선택은 또렷하게 해볼 만해요.|작은 칭찬 하나가 몸의 리듬을 살려줄 수 있어요.|급하게 밀어붙이기보다 한 박자 보고 움직이면 좋아요.|패스 전에 고개를 들면 주변 장면이 더 잘 보여요.|준비운동을 꼼꼼히 하면 플레이가 더 산뜻해져요.|실수 뒤 표정을 빨리 정리하면 다음 장면이 살아나요.|오늘은 정확한 한 번이 강한 세 번보다 잘 맞아요.|친구의 움직임을 먼저 읽으면 좋은 장면이 열려요.|첫 터치가 마음에 들면 그 리듬을 두 번 더 반복해 보세요.|볼을 받기 전 어깨를 한 번 돌리면 선택이 쉬워져요.|짧게 성공한 장면 하나가 하루 분위기를 바꿀 수 있어요.|몸이 무거워도 첫 5분만 가볍게 넘기면 괜찮아져요.|말보다 손짓 하나가 팀 호흡을 빨리 맞춰줘요.|마지막 슈팅보다 그 전 패스가 더 빛날 수 있어요.|오늘은 크게 보여주기보다 깔끔하게 끝내는 쪽이 좋아요.|쉬는 시간에 물 한 모금과 깊은 숨이 도움이 돼요.|공을 놓친 뒤 바로 따라가면 좋은 장면을 다시 만들 수 있어요.|친구에게 먼저 좋은 말을 건네면 플레이도 부드러워져요.|한 번 더 보는 습관이 오늘의 가장 쉬운 무기가 돼요.|가볍게 웃고 시작하면 몸이 빨리 풀릴 수 있어요.|수비할 때는 첫 발을 작게 두면 방향 전환이 편해져요.|공 없는 순간에도 한 걸음 움직이면 공간이 열려요.|오늘의 베스트 장면 하나만 확실히 챙겨보세요.|마무리는 세게보다 정확하게 가져가면 좋아요.';

  @override
  String get fortuneLuckyColorTones =>
      '딥|소프트|클린|선셋|쿨|웜|미스트|브라이트|모노|포인트|네온|파스텔|메탈릭|프레시|차분한|스파클';

  @override
  String get fortuneLuckyColorBases =>
      '네이비|에메랄드|코랄|머스타드|스카이블루|카키|아이보리|체리 레드|라임|차콜|로열 블루|민트|피치|바이올렛|실버|골드|화이트|블랙|올리브|터쿼이즈';

  @override
  String get fortuneLuckyTimePeriods =>
      '이른 오전|오전 후반|점심 직후|초반 오후|늦은 오후|해질 무렵|저녁 초반|밤 루틴 시간|등교 전|쉬는 시간|훈련 시작 전|마무리 스트레칭';
  ...
  String get fortuneLuckyZoneModifiers =>
      '왼쪽|오른쪽|중앙|하프라인 근처|박스 바깥|터치라인 쪽|전진 시작|수비 전환|골대 앞|뒤로 물러난|압박 직전|공 받기 전|스프린트 시작|리턴 패스 후|코너 근처|중앙선 뒤';

  @override
  String get fortuneLuckyZoneBases =>
      '하프스페이스|터치라인 안쪽|첫 터치 지점|리턴 패스 각도|세컨드볼 반응 구역|압박 탈출 출발점|원투패스 연결선|침투 타이밍 구간|시야 확보 자리|마무리 직전 공간|수비 뒤 빈자리|골대 정면 1미터|짧은 패스 삼각형|턴 이후 첫 걸음|친구와 눈 맞추는 자리|크로스 받는 뒷공간|스프린트 출발선|중앙으로 접는 길|느린 호흡을 찾는 자리|마지막 확인 구역';

  @override
  String get fortuneLuckyCueOpenings =>
      '짧게|첫 세트 전에|호흡 고른 뒤|볼을 받기 전에|발끝을 깨운 다음|고개를 든 직후|턴 동작 직전|리듬이 흔들리면|물 마신 다음|친구 이름을 부르고|첫 실수 뒤에|패스가 끊기면|슈팅 전 한 번|수비로 바뀌면|코치 말을 들은 뒤|마무리 스트레칭 전에';

  @override
  String get fortuneLuckyCueActions =>
      '시선 한 번 더 확인하기|터치 방향 먼저 정하기|왼발과 오른발 간격 맞추기|첫 발 디딤을 가볍게 두기|몸을 열고 다음 선택 보기|짧은 호흡으로 템포 묶기|한 번에 세게보다 정확하게 두기|볼 오기 전에 어깨 방향 정리하기|친구 위치를 소리로 알려주기|두 번째 터치를 작게 만들기|공 없는 한 걸음 먼저 움직이기|드리블 뒤 바로 패스 길 보기|실수 뒤 바로 따라가기|슈팅 전 발밑을 한 번 확인하기|리턴 패스 받을 자리 만들기|압박 오기 전 몸 돌리기|짧은 칭찬으로 분위기 올리기|마지막 10분에도 자세 낮추기|세컨드볼을 먼저 예상하기|터치라인 쪽 공간을 확인하기|고개를 든 뒤 속도 올리기|패스 후 멈추지 않고 움직이기|약한 발로 가볍게 연결하기|오늘 좋았던 장면 기억하기';
  ...
  @override
  String get parentReadOnlyFortuneEmpty => '저장된 플레이 힌트가 아직 없어요.';
