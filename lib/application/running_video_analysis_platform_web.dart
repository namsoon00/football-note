...
extension type _RunningVideoPoseAnalyzer._(JSObject _) implements JSObject {
  ...
  external JSPromise<JSAny?> analyzePreviewPose(
    JSUint8Array bytes,
    JSString name,
  );
  external JSPromise<JSAny?> analyzePreviewPoseUrl(
    JSString url,
    JSString name,
  );
}
...
Future<RunningVideoPosePreviewResult> analyzeRunningVideoPreviewPose(
  ...
  try {
    ...
    if (RunningVideoAnalysisService.isReusableBrowserVideoUrl(path)) {
      rawResult =
          await bridge.analyzePreviewPoseUrl(path.toJS, video.name.toJS).toDart;
    } else {
      final bytes = await video.readAsBytes();
      if (bytes.isEmpty) {
        throw const RunningVideoAnalysisException(
      ...
      rawResult =
          await bridge.analyzePreviewPose(bytes.toJS, video.name.toJS).toDart;
    }
    final converted = rawResult.dartify();
    if (converted is! Map) {
      throw const RunningVideoAnalysisException(
        'empty_result',
        'The web analyzer returned no preview pose data.',
      );
    }
    return RunningVideoPosePreviewResult.fromMap(
      converted.map<Object?, Object?>((key, value) => MapEntry(key, value)),
    );
  } on RunningVideoAnalysisException {
    rethrow;
