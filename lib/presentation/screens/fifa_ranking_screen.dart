...
class _FifaRankingScreenState extends State<FifaRankingScreen> {
  ...
  Widget _buildRankingCard(BuildContext context, FifaWorldOverview overview) {
    final l10n = AppLocalizations.of(context)!;
    const sectionId = 'ranking';
    final expanded = _expandedSections.contains(sectionId);
    final canToggle = overview.rankings.length > 12;
    final visibleRankings = canToggle && !expanded
        ? overview.rankings.take(12).toList(growable: false)
        : overview.rankings;
      ...
      child: Column(
        ...
        children: [
          ...
          for (var index = 0; index < visibleRankings.length; index++) ...[
            _RankingRow(
              entry: visibleRankings[index],
              onTap: () => _openCountryDetail(visibleRankings[index]),
            ),
            if (index != visibleRankings.length - 1) const SizedBox(height: 10),
          ],
          if (canToggle) ...[
            const SizedBox(height: 10),
            TextButton.icon(
              onPressed: () => _toggleSection(sectionId),
              icon: Icon(
                expanded
              ...
              label: Text(
                expanded ? l10n.fifaHubCollapseList : l10n.fifaHubShowAllList,
              ),
            ),
          ],
        ],
  ...
  Widget _buildMatchSection(
    ...
    final expanded = _expandedSections.contains(sectionId);
    final initialVisibleCount = sectionId == 'fifa-upcoming' ? 8 : 3;
    final canToggle = matches.length > initialVisibleCount;
    final visibleMatches = canToggle && !expanded
        ? matches.take(initialVisibleCount).toList(growable: false)
        : matches;
    final l10n = AppLocalizations.of(context)!;
    return WatchCartCard(
      child: Column(
          ...
          if (matches.isEmpty)
            ...
            for (var index = 0; index < visibleMatches.length; index++) ...[
              _MatchRow(
                ...
                onTap: () => _openFifaMatchDetail(visibleMatches[index]),
              ),
              if (index != visibleMatches.length - 1)
                const SizedBox(height: 10),
            ],
            if (canToggle) ...[
              const SizedBox(height: 10),
  ...
  Widget _buildKfaMatchSection(
    ...
    final expanded = _expandedSections.contains(sectionId);
    final canToggle = matches.length > 3;
    final visibleMatches = canToggle && !expanded
        ...
        children: [
          ...
          if (matches.isEmpty)
            ...
            for (var index = 0; index < visibleMatches.length; index++) ...[
              _KfaMatchRow(
                match: visibleMatches[index],
                onTap: () => _openKfaMatchDetail(visibleMatches[index]),
              ),
              if (index != visibleMatches.length - 1)
                const SizedBox(height: 10),
            ],
  ...
  Future<void> _loadOverview({bool force = false}) async {
    ...
    final rankingFuture = _captureLoad(
      _service.fetchRankingOverview(gender: _rankingGender),
    );
  ...
  Future<void> _applyRankingLoad(
    int token,
    Future<_LoadResult<FifaWorldOverview>> future,
  ) async {
    ...
    setState(() {
      ...
      if (result.value != null) {
        _overview = _mergeRankingOverview(result.value!, _overview);
      }
  ...
  Future<void> _applyMatchLoad(
    ...
    setState(() {
      ...
      if (result.value != null) {
        _overview = _mergeMatchOverview(result.value!, _overview);
      }
      if (!result.succeeded || result.value == null) {
        _hadError = true;
      }
  ...
  Future<void> _applyKfaLoad(
    int token,
    Future<_LoadResult<KfaMatchOverview>> future,
  ) async {
    final result = await future;
    if (!mounted || token != _loadToken) return;
    setState(() {
      _isKfaLoading = false;
      _kfaRecentResults = result.value?.recentResults ?? const [];
      _kfaUpcomingFixtures = result.value?.upcomingFixtures ?? const [];
      if (!result.succeeded || result.value == null) {
        _hadError = true;
      }
    });
  ...
  FifaWorldOverview _mergeRankingOverview(
    FifaWorldOverview ranking,
    FifaWorldOverview? current,
  ) {
    return ranking.copyWith(
      recentResults: current?.recentResults ?? const <FifaAMatchEntry>[],
      upcomingFixtures: current?.upcomingFixtures ?? const <FifaAMatchEntry>[],
    );
  }

  FifaWorldOverview _mergeMatchOverview(
    FifaWorldOverview match,
    FifaWorldOverview? current,
  ) {
    if (current == null) {
      return match;
    ...
    return current.copyWith(
      lastUpdatedAt: current.lastUpdatedAt ?? match.lastUpdatedAt,
      nextUpdatedAt: match.nextUpdatedAt ?? current.nextUpdatedAt,
