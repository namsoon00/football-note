...
_ConceptMcqSeed _conceptMcqSeed(
  String id,
  int difficulty,
  _QuizCategory category,
  String koStem,
...
List<_FootballQuizQuestion> _conceptMcqExpansionQuestions({
  required String prefix,
  required List<_ConceptMcqSeed> seeds,
}) {
  final questions = <_FootballQuizQuestion>[];
  for (var index = 0; index < seeds.length; index++) {
    final seed = seeds[index];
    final distractors = _conceptMcqDistractors(seeds, index);
    questions.add(
      _sportQuizQuestion(
        id: '${prefix}_expanded_mcq_${seed.id}',
        conceptKey: '${prefix}_expanded_${seed.id}',
        difficulty: seed.difficulty,
        style: _QuestionStyle.multipleChoice,
        category: seed.category,
        koPrompt: seed.koStem,
        enPrompt: seed.enStem,
        options: [
          _FootballQuizOption(koText: seed.koAnswer, enText: seed.enAnswer),
          for (final distractor in distractors)
            _FootballQuizOption(
              koText: distractor.koAnswer,
              enText: distractor.enAnswer,
            ),
        ],
        correctIndex: 0,
        koExplain: seed.koExplain,
...
List<_ConceptMcqSeed> _conceptMcqDistractors(
  List<_ConceptMcqSeed> seeds,
  int index,
) {
  final distractors = <_ConceptMcqSeed>[];
  final seenAnswers = <String>{seeds[index].koAnswer};
  for (final offset in const <int>[7, 17, 29, 1, 3, 5]) {
    final candidate = seeds[(index + offset) % seeds.length];
    if (seenAnswers.add(candidate.koAnswer)) {
...
List<_FootballQuizQuestion> _nonFootballHundredFloorQuestions(
  String sportId,
) {
  return switch (sportId) {
    SportCatalog.baseballId => _conceptMcqExpansionQuestions(
        prefix: 'baseball',
        seeds: _baseballHundredFloorConceptSeeds(),
      ),
    SportCatalog.basketballId => _conceptMcqExpansionQuestions(
        prefix: 'basketball',
        seeds: _basketballHundredFloorConceptSeeds(),
      ),
    SportCatalog.tennisId => _conceptMcqExpansionQuestions(
        prefix: 'tennis',
        seeds: _tennisHundredFloorConceptSeeds(),
      ),
    _ => const <_FootballQuizQuestion>[],
  };
}

List<_ConceptMcqSeed> _baseballHundredFloorConceptSeeds() {
  return <_ConceptMcqSeed>[
    _conceptMcqSeed(
      'first_pitch_strike_leverage',
      2,
      _QuizCategory.tactics,
      '선발 투수가 초반에 스트라이크를 먼저 잡아 타자의 선택지를 줄이는 경기 운영 개념은?',
      'What concept describes a starter using early strikes to reduce hitter options?',
      '초구 스트라이크 레버리지',
      'first-pitch strike leverage',
      '초구 스트라이크는 이후 유인구와 존 경계 승부를 더 설계하기 쉽게 만드는 출발점입니다.',
      'A first-pitch strike makes later chase pitches and edge attacks easier to design.',
      '투구 문항은 공 하나가 다음 두세 개 선택을 어떻게 바꾸는지 보세요.',
      'For pitching questions, read how one pitch changes the next two or three choices.',
    ),
    _conceptMcqSeed(
      'sequencing_speed_ladder',
      3,
      _QuizCategory.tactics,
      '구속대가 다른 공을 계단처럼 배치해 타자의 타이밍을 계속 흔드는 투구 설계는?',
      'What pitch plan layers different velocity bands to keep changing hitter timing?',
      '구속 사다리 시퀀싱',
      'velocity ladder sequencing',
      '빠른 공과 느린 공을 무작정 섞는 것이 아니라 타자의 타이밍 창을 좁히는 순서 설계입니다.',
      'It is not random speed mixing; it narrows the hitter’s timing window through order.',
      '구종 이름보다 직전 공이 다음 공의 체감 속도를 어떻게 만드는지 연결하세요.',
      'Connect the previous pitch to how the next pitch feels, not only to pitch names.',
    ),
    _conceptMcqSeed(
      'chase_rate_discipline',
      2,
      _QuizCategory.technique,
      '타자가 존 밖 유인구에 얼마나 자주 반응하는지 보고 선구안을 평가하는 관점은?',
      'What lens evaluates plate discipline by how often a hitter offers at pitches outside the zone?',
      '체이스율 관리',
      'chase-rate control',
      '체이스율은 단순 삼진 수보다 어떤 공을 참아냈는지 보여 주는 접근 지표입니다.',
      'Chase rate shows what a hitter can take, not only how often they strike out.',
      '타격 문항은 스윙 결과보다 스윙할 공을 고르는 기준을 먼저 보세요.',
      'For hitting, inspect swing selection before the swing result.',
    ),
    _conceptMcqSeed(
      'opposite_field_two_strike',
      2,
      _QuizCategory.mindset,
      '2스트라이크 이후 장타 욕심을 줄이고 바깥쪽 공을 밀어치는 접근은?',
      'What two-strike approach reduces power hunting and drives outside pitches the other way?',
      '밀어치기 콘택트 접근',
      'opposite-field contact approach',
      '2스트라이크에서는 약한 타구라도 인플레이를 만들면 다음 상황으로 이어질 수 있습니다.',
      'With two strikes, even modest contact in play can extend the situation.',
      '카운트가 불리해질수록 목표는 완벽한 타구보다 살아남는 콘택트로 바뀝니다.',
      'As the count worsens, the goal shifts from perfect contact to survivable contact.',
    ),
    _conceptMcqSeed(
      'launch_angle_adjustment',
      2,
      _QuizCategory.technique,
      '땅볼만 반복되는 타자가 스윙 궤도와 임팩트 각도를 조정해 좋은 뜬공을 노리는 개념은?',
      'What concept adjusts swing path and contact angle when a hitter keeps rolling over grounders?',
      '발사각 조정',
      'launch-angle adjustment',
      '발사각은 무조건 크게 띄우는 뜻이 아니라 강한 타구가 적절한 높이로 나가게 하는 조정입니다.',
      'Launch angle is not simply lifting everything; it tunes hard contact into a useful window.',
      '타구 질은 속도와 각도를 함께 봐야 합니다.',
      'Read batted-ball quality through both speed and angle.',
    ),
    _conceptMcqSeed(
      'squeeze_bunt_read',
      3,
      _QuizCategory.tactics,
      '3루 주자가 있는 상황에서 번트와 스타트를 결합해 득점을 노리는 고위험 작전은?',
      'What high-risk tactic combines a bunt and runner break from third to score?',
      '스퀴즈 번트',
      'squeeze bunt',
      '스퀴즈는 번트 정확도, 투수 움직임, 3루 주자 스타트가 모두 맞아야 합니다.',
      'A squeeze needs bunt precision, pitcher read, and the runner’s break to align.',
      '작전 문항은 성공 장면보다 실패했을 때의 리스크까지 같이 판단하세요.',
      'For tactics, judge the failure risk as well as the success picture.',
    ),
    _conceptMcqSeed(
      'delayed_steal_timing',
      2,
      _QuizCategory.tactics,
      '포수의 느슨한 리턴이나 내야수 방심을 이용해 한 박자 늦게 출발하는 주루 작전은?',
      'What baserunning tactic leaves a beat late to exploit a casual return or inattentive infield?',
      '딜레이드 스틸',
      'delayed steal',
      '딜레이드 스틸은 순수 속도보다 수비 집중과 송구 루틴의 빈틈을 노립니다.',
      'A delayed steal attacks defensive attention and throw routine more than raw speed.',
      '주루는 스타트 속도뿐 아니라 수비가 공을 놓는 순간을 읽어야 합니다.',
      'Baserunning reads when the defense relaxes, not only how fast the runner starts.',
    ),
    _conceptMcqSeed(
      'first_to_third_read',
      2,
      _QuizCategory.positions,
      '1루 주자가 안타 때 3루까지 갈지 판단하며 외야수 위치와 타구 방향을 함께 읽는 기술은?',
      'What skill reads outfielder depth and batted-ball direction to decide going first to third?',
      '퍼스트 투 서드 판단',
      'first-to-third read',
      '좋은 주자는 공이 빠지는 위치와 외야수 어깨, 3루 코치 신호를 동시에 봅니다.',
      'A good runner reads ball location, outfielder arm, and third-base coach together.',
      '진루 판단은 공만 보는 것이 아니라 다음 송구 각도까지 보는 일입니다.',
      'Advancement reads the next throw angle, not only the ball.',
    ),
    _conceptMcqSeed(
      'relay_trailer_communication',
      2,
      _QuizCategory.positions,
      '외야 깊은 타구에서 중계 뒤를 받쳐 나쁜 송구나 다음 주자를 대비하는 선수 역할은?',
      'What role backs up a relay on a deep outfield ball to cover a bad throw or next runner?',
      '트레일러 백업',
      'trailer backup',
      '트레일러는 눈에 띄지 않아도 추가 진루와 실점을 막는 보험 역할을 합니다.',
      'The trailer is quiet insurance against extra bases and throwing mistakes.',
      '수비 문항은 공을 잡는 선수뿐 아니라 뒤를 받치는 선수까지 보세요.',
      'Defensive questions should include the player backing up the play.',
    ),
    _conceptMcqSeed(
      'pitcher_fielding_position',
      1,
      _QuizCategory.positions,
      '투구 후 투수가 번트, 1루 커버, 홈 송구에 대비해 수비수로 전환하는 기본 책임은?',
      'What responsibility turns the pitcher into a fielder after delivery for bunts, first-base cover, or home throws?',
      '투수 수비 위치 전환',
      'pitcher fielding position',
      '투수는 던지고 끝나는 포지션이 아니라 타구와 베이스 커버에 즉시 반응해야 합니다.',
      'The pitcher’s job does not end at release; they must react to batted balls and base cover.',
      '투수 수비는 투구 뒤 첫 두 걸음의 방향으로 시작합니다.',
      'Pitcher defense starts with the direction of the first two steps after release.',
    ),
    _conceptMcqSeed(
      'catcher_blocking_angle',
      2,
      _QuizCategory.technique,
      '폭투성 공을 몸 앞으로 떨어뜨리기 위해 포수가 무릎과 가슴 각도를 낮추는 기술은?',
      'What catching skill lowers knee and chest angle to deaden a pitch in front?',
      '블로킹 각도',
      'blocking angle',
      '블로킹은 공을 잡는 기술보다 공을 멀리 튀지 않게 죽이는 기술에 가깝습니다.',
      'Blocking is closer to killing the ball than catching it cleanly.',
      '포수 기술은 글러브보다 몸이 공을 어디로 떨어뜨리는지 확인하세요.',
      'For catching, check where the body makes the ball drop, not only the glove.',
    ),
    _conceptMcqSeed(
      'quiet_framing_receive',
      2,
      _QuizCategory.technique,
      '포수가 낮은 공을 크게 끌어올리지 않고 조용히 받아 스트라이크 가능성을 높이는 기술은?',
      'What skill presents a low pitch quietly instead of yanking it upward?',
      '조용한 프레이밍',
      'quiet framing',
      '좋은 프레이밍은 과장된 움직임보다 안정된 몸과 작은 글러브 이동으로 판정 신뢰를 만듭니다.',
      'Good framing earns trust through body stability and small glove movement, not exaggeration.',
      '포구 후 움직임이 클수록 심판에게 조작처럼 보일 수 있습니다.',
      'Large post-catch movement can look manipulated to the umpire.',
    ),
    _conceptMcqSeed(
      'wild_pitch_vs_passed_ball',
      1,
      _QuizCategory.rules,
      '투구가 빠진 책임을 투수와 포수 중 어디에 둘지 구분하는 기록 판단은?',
      'What scoring judgment separates pitcher responsibility from catcher responsibility on a missed pitch?',
      '폭투와 패스트볼 구분',
      'wild pitch versus passed ball',
      '기록 판단은 공이 정상적으로 잡을 수 있었는지와 투구 난도를 함께 봅니다.',
      'The scoring judgment weighs whether the pitch was reasonably catchable and how difficult it was.',
      '규칙 문항은 용어보다 책임 기준을 함께 확인하세요.',
      'For rules, connect the term to the responsibility standard.',
    ),
    _conceptMcqSeed(
      'leverage_index_bullpen',
      3,
      _QuizCategory.tactics,
      '마무리 이닝보다 경기에서 가장 중요한 아웃이 언제인지 보고 불펜을 쓰는 지표 관점은?',
      'What metric lens uses relievers when the most important outs arrive, not only in the ninth?',
      '레버리지 인덱스',
      'leverage index',
      '레버리지 인덱스는 이닝 이름보다 점수, 주자, 아웃이 만드는 압박도를 봅니다.',
      'Leverage index reads pressure from score, runners, and outs rather than inning labels.',
      '투수 운용은 누가 마무리인지보다 지금 아웃의 가치부터 보세요.',
      'Pitching management starts with the value of the current out, not only closer labels.',
    ),
    _conceptMcqSeed(
      'opener_strategy',
      3,
      _QuizCategory.tactics,
      '짧은 이닝을 던지는 투수를 먼저 내세워 상위 타순 매치업을 조절하는 운용 방식은?',
      'What pitching plan starts a short-stint arm to manage top-order matchups?',
      '오프너 전략',
      'opener strategy',
      '오프너는 선발의 전통적 길이보다 첫 타순 매치업과 불펜 연결을 우선합니다.',
      'An opener prioritizes first-lineup matchups and bullpen flow over traditional starter length.',
      '투수 역할은 선발/불펜 이름보다 몇 번째 타순을 상대하는지로도 평가하세요.',
      'Judge pitching roles by which lineup segment they face, not only starter or bullpen labels.',
    ),
    _conceptMcqSeed(
      'platoon_advantage',
      2,
      _QuizCategory.tactics,
      '좌우 타자와 투수의 상성 차이를 이용해 대타나 투수 교체를 결정하는 기준은?',
      'What matchup idea uses handedness to guide pinch-hitting or pitching changes?',
      '플래툰 어드밴티지',
      'platoon advantage',
      '플래툰은 단순 좌우 암기가 아니라 선수별 스플릿과 구종 상성까지 함께 봐야 합니다.',
      'Platoon thinking should include player splits and pitch matchup, not handedness alone.',
      '교체 문항은 좌우와 함께 실제 성향 데이터를 같이 확인하세요.',
      'For substitutions, pair handedness with actual tendency data.',
    ),
    _conceptMcqSeed(
      'shift_restriction_range',
      2,
      _QuizCategory.rules,
      '시프트 제한 이후 내야수가 더 직접적으로 평가받게 된 수비 능력은?',
      'After shift restrictions, which defensive skill became more directly exposed for infielders?',
      '개별 수비 범위',
      'individual defensive range',
      '포지셔닝으로 가리던 영역이 줄어들면 첫 스텝과 좌우 범위의 가치가 커집니다.',
      'When positioning help is reduced, first step and lateral range become more valuable.',
      '규칙 변화는 선수 평가 기준 변화까지 연결해서 보세요.',
      'Connect rule changes to how players are evaluated.',
    ),
    _conceptMcqSeed(
      'no_doubles_outfield_depth',
      2,
      _QuizCategory.positions,
      '경기 후반 장타만은 막아야 할 때 외야수가 평소보다 깊게 서는 수비 선택은?',
      'What outfield choice plays deeper late in games when preventing extra-base hits matters most?',
      '노 더블스 수비',
      'no-doubles defense',
      '노 더블스 수비는 짧은 안타 위험을 감수하고 뒤 공간 장타를 줄이는 선택입니다.',
      'No-doubles defense accepts more singles to reduce balls over the outfielders.',
      '수비 깊이는 점수 차와 남은 아웃을 함께 봐야 합니다.',
      'Defensive depth depends on score margin and remaining outs.',
    ),
    _conceptMcqSeed(
      'run_expectancy_matrix',
      3,
      _QuizCategory.tactics,
      '주자와 아웃 조합별 평균 득점 가능성을 바탕으로 번트와 강공을 비교하는 도구는?',
      'What tool compares bunting and swinging through average runs by base-out state?',
      '득점 기대값 매트릭스',
      'run expectancy matrix',
      '득점 기대값은 감으로 작전을 고르지 않고 상황별 평균 결과를 비교하게 해 줍니다.',
      'Run expectancy lets tactics compare average outcomes instead of relying only on feel.',
      '작전 선택은 한 점이 필요한지 빅이닝이 필요한지와 함께 판단하세요.',
      'Judge tactics together with whether one run or a big inning is needed.',
    ),
    _conceptMcqSeed(
      'win_probability_small_ball',
      3,
      _QuizCategory.tactics,
      '동점 9회처럼 한 점의 가치가 큰 상황에서 기대 득점보다 승리 확률을 더 직접적으로 보는 관점은?',
      'In a tied ninth, what lens may weigh winning the game more directly than average runs?',
      '승리 확률 관점',
      'win-probability lens',
      '평균 득점이 낮아져도 특정 점수 상황에서는 한 점을 얻는 가치가 더 커질 수 있습니다.',
      'Even if average runs drop, one run can matter more in a specific score state.',
      '야구 작전은 평균과 현재 점수 상황을 분리해서 비교하세요.',
      'Separate average value from the current score situation.',
    ),
    _conceptMcqSeed(
      'mound_reset_routine',
      1,
      _QuizCategory.mindset,
      '연속 안타 후 투수가 호흡과 사인 확인으로 다음 공에 주의를 되돌리는 행동은?',
      'What routine brings a pitcher back to the next pitch after consecutive hits?',
      '마운드 리셋 루틴',
      'mound reset routine',
      '리셋 루틴은 감정을 없애는 것이 아니라 다음 실행 단서로 주의를 옮기는 과정입니다.',
      'A reset routine does not erase emotion; it moves attention to the next execution cue.',
      '마인드 문항은 결과보다 다음 행동 단서를 찾으세요.',
      'Mindset questions should find the next action cue, not only the result.',
    ),
    _conceptMcqSeed(
      'secondary_lead_reaction',
      2,
      _QuizCategory.technique,
      '투구가 홈으로 향한 뒤 다음 베이스와 타구 반응을 준비하는 주자의 두 번째 움직임은?',
      'What second movement prepares a runner for the next base or batted-ball read after delivery?',
      '세컨더리 리드',
      'secondary lead',
      '세컨더리 리드는 무작정 멀어지는 것이 아니라 반응 준비와 귀루 균형을 함께 만드는 동작입니다.',
      'A secondary lead is not drifting away; it balances reaction readiness and return safety.',
      '리드 폭보다 다음 반응을 시작할 수 있는 자세를 보세요.',
      'Watch posture for the next reaction before lead size.',
    ),
    _conceptMcqSeed(
      'pickoff_move_read',
      2,
      _QuizCategory.technique,
      '주자가 투수의 어깨, 발, 리듬을 보고 견제인지 투구인지 구분하는 능력은?',
      'What skill reads shoulder, feet, and rhythm to separate pickoff from delivery?',
      '견제 동작 읽기',
      'pickoff-move read',
      '견제 읽기는 출발 욕심보다 돌아갈 수 있는 균형과 투수 습관 관찰이 먼저입니다.',
      'Reading pickoffs starts with return balance and pitcher habits before stealing ambition.',
      '주루 기술은 빠른 출발과 안전한 귀루를 같이 훈련해야 합니다.',
      'Baserunning skill trains fast starts and safe returns together.',
    ),
    _conceptMcqSeed(
      'tag_play_footwork',
      2,
      _QuizCategory.technique,
      '야수가 송구를 받으며 베이스 앞쪽에서 주자의 슬라이드 길을 줄이는 태그 준비는?',
      'What footwork lets a fielder receive and shrink the runner’s slide lane before tagging?',
      '태그 플레이 풋워크',
      'tag-play footwork',
      '좋은 태그는 손만 빠른 것이 아니라 포구 위치와 발 위치가 태그 거리를 줄입니다.',
      'A good tag is not only quick hands; catch location and feet shorten tag distance.',
      '태그 상황은 공, 베이스, 주자 몸 방향을 삼각형으로 보세요.',
      'Read tag plays as a triangle of ball, base, and runner body line.',
    ),
    _conceptMcqSeed(
      'force_play_priority',
      1,
      _QuizCategory.rules,
      '수비가 태그 없이 베이스만 밟아 아웃을 만들 수 있는지 먼저 판단하는 기준은?',
      'What standard tells the defense whether touching the base can record an out without a tag?',
      '포스 플레이 우선 판단',
      'force-play priority',
      '포스 상황을 먼저 알면 송구 목표와 태그 필요 여부를 빠르게 결정할 수 있습니다.',
      'Knowing the force first quickly sets the throw target and whether a tag is needed.',
      '아웃카운트와 주자 배치를 투구 전에 말하는 습관을 들이세요.',
      'Build the habit of calling outs and runner setup before the pitch.',
    ),
    _conceptMcqSeed(
      'double_steal_communication',
      3,
      _QuizCategory.tactics,
      '두 주자가 동시에 움직일 때 포수와 내야가 먼저 잡을 주자를 콜로 정하는 수비 원칙은?',
      'What defensive principle calls which runner to attack first when two runners break?',
      '더블 스틸 커뮤니케이션',
      'double-steal communication',
      '더블 스틸 수비는 강한 송구보다 우선순위 콜과 중간 컷 판단이 먼저입니다.',
      'Defending a double steal starts with priority calls and cut decisions before arm strength.',
      '복수 주자 상황은 가장 큰 실점 위험이 어디인지 먼저 정하세요.',
      'With multiple runners, decide where the largest run risk is first.',
    ),
    _conceptMcqSeed(
      'doubleheader_fuel_plan',
      2,
      _QuizCategory.nutrition,
      '더블헤더처럼 긴 경기일에 한 번에 많이 먹기보다 이닝 사이 작은 보충을 계획하는 방식은?',
      'What fueling plan uses small between-inning intake instead of one large meal on a doubleheader day?',
      '분할 보충 전략',
      'split fueling strategy',
      '긴 경기일에는 위 부담을 줄이면서 탄수화물과 수분을 끊기지 않게 넣는 계획이 중요합니다.',
      'Long game days need steady carbohydrate and fluid without overloading the gut.',
      '영양 문항은 음식 이름보다 타이밍과 소화 부담을 함께 보세요.',
      'Nutrition questions should pair timing with digestion load.',
    ),
    _conceptMcqSeed(
      'dugout_hydration_routine',
      1,
      _QuizCategory.nutrition,
      '갈증이 오기 전에 이닝 사이 반복적으로 수분을 보충하는 선수 루틴은?',
      'What player routine drinks between innings before thirst becomes obvious?',
      '더그아웃 수분 루틴',
      'dugout hydration routine',
      '수분 루틴은 더위와 땀 양이 큰 날 후반 집중력 저하를 줄이는 기본 장치입니다.',
      'A hydration routine protects late-game focus on hot, high-sweat days.',
      '수분은 한 번에 많이보다 자주 확인하는 루틴이 더 안정적입니다.',
      'Hydration is steadier as repeated checks than as one large intake.',
    ),
    _conceptMcqSeed(
      'shoulder_recovery_care',
      2,
      _QuizCategory.training,
      '투수나 포수가 던진 뒤 가동성, 가벼운 순환, 통증 신호를 확인하는 회복 관리는?',
      'What recovery care checks mobility, light circulation, and pain signs after heavy throwing?',
      '어깨 회복 관리',
      'shoulder recovery care',
      '던지는 선수는 힘을 키우는 훈련만큼 회복 신호를 기록하는 습관이 중요합니다.',
      'Throwers need the habit of tracking recovery signs as much as building strength.',
      '훈련 문항은 오늘 던진 양과 내일 팔 상태를 연결하세요.',
      'Training questions should connect today’s throws with tomorrow’s arm status.',
    ),
    _conceptMcqSeed(
      'workload_pitch_count',
      2,
      _QuizCategory.training,
      '어린 투수 보호를 위해 경기 투구 수와 휴식일을 함께 관리하는 기준은?',
      'What standard manages game pitches and rest days together to protect young pitchers?',
      '투구 수 부하 관리',
      'pitch-count workload management',
      '투구 수는 단순 숫자가 아니라 피로 누적과 회복 시간을 조절하는 안전 장치입니다.',
      'Pitch count is a safety tool for fatigue accumulation and recovery time, not only a number.',
      '부하 관리는 오늘 기록보다 다음 등판 품질을 지키는 일입니다.',
      'Load management protects the quality of the next outing.',
    ),
    _conceptMcqSeed(
      'scouting_report_attack_plan',
      3,
      _QuizCategory.tactics,
      '타자 약점, 투수 구종, 수비 위치를 미리 묶어 경기 전 승부 방식을 정리하는 자료는?',
      'What pregame material links hitter weaknesses, pitch mix, and defensive positioning into an attack plan?',
      '스카우팅 리포트',
      'scouting report',
      '스카우팅 리포트는 정보 목록이 아니라 첫 선택을 빠르게 만드는 실행 계획이어야 합니다.',
      'A scouting report should be an action plan for first choices, not a list of facts.',
      '자료 문항은 정보가 어떤 경기 행동으로 바뀌는지 확인하세요.',
      'Information questions should ask what game action the report changes.',
    ),
    _conceptMcqSeed(
      'fielding_independent_pitching',
      3,
      _QuizCategory.fun,
      '수비 영향이 섞인 실점보다 삼진, 볼넷, 홈런처럼 투수가 통제한 결과를 보려는 지표 관점은?',
      'What metric lens looks at strikeouts, walks, and homers more than defense-influenced runs?',
      '수비 무관 투구 지표',
      'fielding-independent pitching',
      'FIP 계열 지표는 투수 책임을 수비 결과와 분리해 보려는 분석 관점입니다.',
      'FIP-style metrics try to separate pitcher responsibility from defensive outcomes.',
      '기록 문항은 숫자가 무엇을 제외하고 무엇을 남기는지 물어야 합니다.',
      'Metric questions should ask what the number excludes and keeps.',
    ),
    _conceptMcqSeed(
      'quality_start_context',
      1,
      _QuizCategory.fun,
      '선발 투수가 일정 이닝 이상을 던지고 실점을 제한했는지 보는 전통적 경기 평가 용어는?',
      'What traditional term evaluates whether a starter worked enough innings while limiting runs?',
      '퀄리티 스타트',
      'quality start',
      '퀄리티 스타트는 선발의 최소 안정성을 보는 지표지만 경기 맥락과 함께 해석해야 합니다.',
      'A quality start shows baseline starter stability but still needs game context.',
      '기록은 기준을 아는 것에서 끝내지 말고 한계까지 같이 보세요.',
      'Know the threshold and the limitation of the stat.',
    ),
    _conceptMcqSeed(
      'barrel_contact',
      2,
      _QuizCategory.technique,
      '강한 타구를 만들기 위해 배트의 가장 효율적인 부위로 공을 맞히는 접촉 개념은?',
      'What contact concept uses the most efficient part of the bat to create hard contact?',
      '배럴 콘택트',
      'barrel contact',
      '배럴 콘택트는 힘만 쓰는 것이 아니라 타이밍과 스윙 궤도가 맞아야 나옵니다.',
      'Barrel contact comes from timing and swing path, not force alone.',
      '타격 기술은 세게 치기보다 좋은 접촉 지점을 반복하는 일입니다.',
      'Hitting skill repeats the right contact point more than swinging harder.',
    ),
    _conceptMcqSeed(
      'defensive_first_step',
      2,
      _QuizCategory.technique,
      '내야수가 타구 순간 첫 반응 방향을 빠르게 잡아 수비 범위를 넓히는 능력은?',
      'What ability lets an infielder expand range by choosing the first movement direction at contact?',
      '첫 스텝 반응',
      'first-step reaction',
      '수비 범위는 최고 속도보다 타구 판단 직후 첫 스텝이 크게 좌우합니다.',
      'Defensive range is shaped heavily by the first step after contact read, not only top speed.',
      '수비 훈련은 공을 잡은 뒤보다 출발 순간을 먼저 보세요.',
      'Defensive training should inspect the start before the catch.',
    ),
    _conceptMcqSeed(
      'catcher_game_calling',
      3,
      _QuizCategory.tactics,
      '타자 반응, 투수 강점, 주자 상황을 묶어 포수가 다음 구종을 설계하는 역할은?',
      'What catcher role blends hitter reaction, pitcher strength, and base state to choose the next pitch?',
      '게임 콜링',
      'game calling',
      '게임 콜링은 정답 구종을 외우는 것이 아니라 지금 타자가 무엇을 보였는지 읽는 과정입니다.',
      'Game calling reads what the hitter has shown now rather than memorizing one right pitch.',
      '배터리 문항은 투수와 포수가 같은 의도를 공유하는지 보세요.',
      'Battery questions should ask whether pitcher and catcher share the same intent.',
    ),
    _conceptMcqSeed(
      'backup_first_base',
      1,
      _QuizCategory.positions,
      '내야 땅볼 때 투수가 1루 뒤쪽을 받쳐 악송구 추가 진루를 막는 움직임은?',
      'What movement has the pitcher back up first base on an infield grounder to prevent extra bases?',
      '1루 백업',
      'first-base backup',
      '백업 플레이는 하이라이트에 적게 나오지만 실책 뒤 큰 손실을 줄입니다.',
      'Backup plays rarely make highlights but reduce damage after errors.',
      '수비 문항은 직접 아웃을 잡는 선수와 보험 위치를 같이 보세요.',
      'Defensive questions should include both the out maker and insurance position.',
    ),
    _conceptMcqSeed(
      'hit_and_run_contact',
      2,
      _QuizCategory.tactics,
      '주자가 먼저 움직이고 타자가 콘택트로 수비 이동을 흔드는 작전은?',
      'What tactic sends the runner early while the hitter makes contact to disturb defensive movement?',
      '히트 앤드 런',
      'hit and run',
      '히트 앤드 런은 주자 속도, 타자 콘택트 능력, 카운트가 맞을 때 효과가 커집니다.',
      'Hit and run works best when runner speed, hitter contact, and count align.',
      '작전 문항은 한 선수 능력보다 두 선수의 타이밍을 함께 보세요.',
      'Tactic questions should read timing between two players, not one skill alone.',
    ),
    _conceptMcqSeed(
      'runner_on_third_contact_play',
      3,
      _QuizCategory.tactics,
      '3루 주자가 땅볼 타구 방향과 스타트 콜에 맞춰 홈 승부를 선택하는 작전은?',
      'What play lets a runner on third break home based on ground-ball direction and a start call?',
      '컨택트 플레이',
      'contact play',
      '컨택트 플레이는 타구 질과 내야 위치를 미리 정해 둔 콜에 따라 반응합니다.',
      'A contact play reacts to batted-ball quality and infield depth through a pre-set call.',
      '3루 작전은 타구 방향, 내야 깊이, 점수 차를 함께 판단하세요.',
      'Runner-on-third tactics combine direction, infield depth, and score.',
    ),
    _conceptMcqSeed(
      'pitch_clock_routine',
      1,
      _QuizCategory.rules,
      '투수와 타자가 제한 시간 안에 준비하도록 해 경기 템포를 높이는 현대 규칙은?',
      'What modern rule forces pitcher and hitter readiness within a time limit to increase pace?',
      '피치 클록',
      'pitch clock',
      '피치 클록은 단순 시간 단축이 아니라 루틴을 짧고 반복 가능하게 바꾸는 규칙입니다.',
      'The pitch clock shortens time and forces routines to become repeatable and compact.',
      '규칙 변화는 선수 루틴과 의사결정 속도에 어떤 영향을 주는지 보세요.',
      'Rule changes should be tied to routine and decision speed.',
    ),
    _conceptMcqSeed(
      'defensive_alignment_card',
      2,
      _QuizCategory.positions,
      '타자 성향별로 외야와 내야 위치를 미리 조정하기 위해 수비가 참고하는 계획은?',
      'What plan helps defenders pre-adjust infield and outfield spots by hitter tendency?',
      '수비 위치 카드',
      'defensive alignment card',
      '수비 위치 카드는 데이터와 현장 판단을 연결해 첫 걸음 전에 유리한 위치를 만듭니다.',
      'An alignment card connects data with field judgment before the first step.',
      '위치 선정은 공이 오기 전 이미 시작되는 수비 기술입니다.',
      'Positioning is a defensive skill that starts before the ball is hit.',
    ),
    _conceptMcqSeed(
      'two_way_player_load',
      3,
      _QuizCategory.training,
      '투타를 병행하는 선수가 타격, 투구, 회복 부하를 한꺼번에 관리해야 하는 이유는?',
      'Why must a two-way player manage hitting, pitching, and recovery load together?',
      '투웨이 부하 관리',
      'two-way load management',
      '투웨이 선수는 한 역할의 피로가 다른 역할의 기술 품질까지 떨어뜨릴 수 있습니다.',
      'For two-way players, fatigue in one role can reduce skill quality in the other.',
      '훈련 문항은 역할별 부하가 서로 어떻게 누적되는지 보세요.',
      'Training questions should read how role loads accumulate together.',
    ),
    _conceptMcqSeed(
      'mental_at_bat_plan',
      1,
      _QuizCategory.mindset,
      '타석 전 노릴 존과 버릴 공을 미리 정해 흔들림을 줄이는 준비는?',
      'What preparation sets the attack zone and take zone before an at-bat to reduce hesitation?',
      '타석 접근 계획',
      'at-bat approach plan',
      '좋은 타석은 공이 온 뒤 결정하는 것이 아니라 미리 세운 기준으로 반응합니다.',
      'A good at-bat reacts from a pre-set standard rather than deciding after the pitch arrives.',
      '마인드 문항은 자신감보다 선택 기준을 분명히 하는지 보세요.',
      'Mindset questions should check clarity of selection standards before confidence.',
    ),
  ];
}

List<_ConceptMcqSeed> _basketballHundredFloorConceptSeeds() {
  return <_ConceptMcqSeed>[
    _conceptMcqSeed(
      'nail_help_position',
      2,
      _QuizCategory.tactics,
      '드라이브를 막기 위해 자유투 라인 중앙 근처에서 잠깐 도움을 주는 수비 위치는?',
      'What help spot near the middle of the free-throw line briefly slows drives?',
      '네일 도움 수비',
      'nail help defense',
      '네일 도움은 볼 핸들러의 직선 돌파를 늦추면서도 자기 매치업으로 복귀할 각을 남깁니다.',
      'Nail help slows a straight-line drive while preserving the recovery angle.',
      '도움 수비는 오래 머무는 것보다 언제 보여 주고 언제 돌아가는지가 중요합니다.',
      'Help defense is about when to show and recover, not staying forever.',
    ),
    _conceptMcqSeed(
      'low_man_rotation',
      2,
      _QuizCategory.tactics,
      '약측 아래에서 롤맨이나 림 돌파를 가장 먼저 막아야 하는 수비 역할은?',
      'What weak-side low defender is first responsible for stopping the roller or rim drive?',
      '로우맨 로테이션',
      'low-man rotation',
      '로우맨은 마지막 순간의 림 보호와 코너 슈터 포기 위험을 동시에 판단해야 합니다.',
      'The low man balances rim protection with the risk of leaving a corner shooter.',
      '픽앤롤 수비는 공 주변 두 명보다 약측 아래 수비까지 같이 보세요.',
      'Pick-and-roll defense includes the weak-side low defender, not only the two at the ball.',
    ),
    _conceptMcqSeed(
      'tag_the_roller',
      2,
      _QuizCategory.tactics,
      '픽앤롤에서 굴러가는 빅맨을 잠깐 몸으로 지연시키고 복귀하는 약측 수비 행동은?',
      'What weak-side action briefly bumps the rolling big before recovering?',
      '롤맨 태그',
      'tagging the roller',
      '롤맨 태그는 쉬운 덩크를 막지만 너무 길면 코너나 윙 슛을 열어 줍니다.',
      'Tagging prevents easy dunks, but holding too long opens corner or wing shots.',
      '도움 수비 문항은 지연과 복귀의 균형을 함께 판단하세요.',
      'For help defense, judge both delay and recovery.',
    ),
    _conceptMcqSeed(
      'drop_coverage_angle',
      2,
      _QuizCategory.tactics,
      '빅맨이 뒤로 물러서며 림을 지키고 가드가 볼을 따라가는 픽앤롤 수비는?',
      'What pick-and-roll coverage has the big retreat to protect the rim while the guard follows the ball?',
      '드롭 커버리지',
      'drop coverage',
      '드롭은 림을 지키는 대신 풀업 점퍼와 플로터 공간을 허용할 수 있습니다.',
      'Drop protects the rim but can concede pull-ups and floaters.',
      '커버리지 문항은 막는 공간과 내주는 공간을 동시에 보세요.',
      'Coverage questions should ask what is protected and what is conceded.',
    ),
    _conceptMcqSeed(
      'switch_peel_communication',
      3,
      _QuizCategory.tactics,
      '스위치 뒤 미스매치가 생겼을 때 뒤 선수들이 다시 매치를 넘겨 균형을 찾는 콜은?',
      'What communication restores matchups after a switch creates a mismatch?',
      '스위치 후 필백 콜',
      'peel-switch communication',
      '좋은 스위치는 바꾸는 순간보다 바꾼 뒤 누가 림과 슈터를 맡는지까지 정리합니다.',
      'A good switch includes the cleanup after the exchange: rim, shooter, and mismatch.',
      '수비 콜은 한 번 바꾸고 끝이 아니라 다음 로테이션까지 이어져야 합니다.',
      'A defensive call continues into the next rotation after the switch.',
    ),
    _conceptMcqSeed(
      'hedge_recover_timing',
      3,
      _QuizCategory.tactics,
      '스크린 수비수가 볼 핸들러 앞에 튀어나와 시간을 벌고 원래 선수에게 돌아가는 수비는?',
      'What coverage has the screen defender step out to delay the ball then recover back?',
      '헤지 앤 리커버',
      'hedge and recover',
      '헤지는 볼을 옆으로 밀어 시간을 벌지만 복귀가 늦으면 롤맨이나 쇼트롤을 내줍니다.',
      'A hedge buys time by pushing the ball sideways, but late recovery opens the roller.',
      '헤지 문항은 튀어나오는 높이와 돌아가는 길을 함께 확인하세요.',
      'Hedge questions should pair show height with the recovery path.',
    ),
    _conceptMcqSeed(
      'ice_side_pick_roll',
      3,
      _QuizCategory.tactics,
      '사이드 픽앤롤에서 볼을 중앙으로 못 쓰게 하고 베이스라인 쪽으로 몰아넣는 수비는?',
      'What side pick-and-roll coverage denies the middle and sends the ball toward the baseline?',
      '아이스 수비',
      'ICE coverage',
      '아이스는 중앙 침투와 스크린 사용을 막고 도움 수비가 준비된 쪽으로 유도합니다.',
      'ICE denies the middle and screen use, sending the ball toward prepared help.',
      '사이드 픽앤롤은 수비가 어느 방향을 지우는지 먼저 보세요.',
      'On side pick-and-roll, first read which direction the defense removes.',
    ),
    _conceptMcqSeed(
      'ghost_screen_slip',
      2,
      _QuizCategory.tactics,
      '스크린을 거는 척하다 접촉 전에 빠져나가 수비 스위치 타이밍을 흔드는 공격 행동은?',
      'What action fakes a screen and slips out before contact to disrupt switch timing?',
      '고스트 스크린',
      'ghost screen',
      '고스트 스크린은 실제 충돌보다 수비의 예상과 콜 타이밍을 공격합니다.',
      'A ghost screen attacks defensive expectation and call timing more than contact.',
      '스크린 문항은 몸으로 막았는지보다 수비 결정을 늦췄는지 보세요.',
      'Screen questions should ask whether the action delayed defensive decisions.',
    ),
    _conceptMcqSeed(
      'short_roll_playmaking',
      3,
      _QuizCategory.tactics,
      '롤맨이 림까지 끝까지 가지 않고 중간에서 받아 4대3 패스 선택을 하는 공격 역할은?',
      'What role catches in the middle of the floor to playmake a 4-on-3 instead of rolling all the way?',
      '쇼트롤 플레이메이킹',
      'short-roll playmaking',
      '쇼트롤은 수비 두 명이 볼에 붙었을 때 중간 공간에서 패스 허브가 되는 선택입니다.',
      'Short roll becomes a passing hub when two defenders commit to the ball.',
      '픽앤롤 문항은 득점자뿐 아니라 중간 연결자를 찾으세요.',
      'Pick-and-roll questions should identify the middle connector, not only the scorer.',
    ),
    _conceptMcqSeed(
      'dunker_spot_spacing',
      2,
      _QuizCategory.positions,
      '림 옆 베이스라인 공간에서 도움 수비 뒤를 점유해 덤프패스와 리바운드를 노리는 위치는?',
      'What baseline spot near the rim occupies space behind help for dump-offs and rebounds?',
      '덩커 스팟',
      'dunker spot',
      '덩커 스팟은 슈팅 공간은 아니지만 림 보호 수비를 묶어 두는 압박 위치입니다.',
      'The dunker spot is not spacing by shooting, but by occupying rim help.',
      '공격 위치는 3점 라인 밖뿐 아니라 림 주변 빈 공간도 포함합니다.',
      'Offensive spacing includes pockets near the rim, not only the three-point line.',
    ),
    _conceptMcqSeed(
      'weakside_lift',
      2,
      _QuizCategory.positions,
      '코너 선수가 윙으로 올라와 도움 수비의 복귀 거리를 늘리는 약측 움직임은?',
      'What weak-side movement lifts a corner player to the wing to lengthen help recovery?',
      '약측 리프트',
      'weak-side lift',
      '리프트는 패스 각도를 만들고 코너 도움 수비가 한 번에 두 명을 막지 못하게 합니다.',
      'A lift creates a passing angle and stops one helper from guarding two players.',
      '스페이싱 문항은 멈춰 있는 위치보다 패스 각도가 어떻게 바뀌는지 보세요.',
      'Spacing questions should read how the passing angle changes.',
    ),
    _conceptMcqSeed(
      'forty_five_cut',
      2,
      _QuizCategory.tactics,
      '윙에서 림을 향해 대각선으로 파고들어 도움 수비 뒤를 찌르는 컷은?',
      'What diagonal wing cut attacks behind help defense toward the rim?',
      '45도 컷',
      '45 cut',
      '45도 컷은 수비가 공을 보며 몸을 돌린 순간 뒤 공간을 공격합니다.',
      'The 45 cut attacks the defender’s back when they turn toward the ball.',
      '컷 문항은 언제 뛰는지가 어디로 뛰는지만큼 중요합니다.',
      'Cutting questions should weigh timing as much as direction.',
    ),
    _conceptMcqSeed(
      'flare_screen_shooter',
      2,
      _QuizCategory.tactics,
      '슈터가 공에서 멀어지는 방향으로 빠질 때 뒤에서 걸어 주는 스크린은?',
      'What screen frees a shooter moving away from the ball toward space?',
      '플레어 스크린',
      'flare screen',
      '플레어는 수비 시야 밖에서 슈터를 코너나 윙 공간으로 풀어 주는 움직임입니다.',
      'A flare frees a shooter into corner or wing space outside the defender’s vision.',
      '슈터 플레이는 공을 향해 오는 움직임과 멀어지는 움직임을 구분하세요.',
      'Separate shooter movement toward the ball from movement away from it.',
    ),
    _conceptMcqSeed(
      'stagger_screen_read',
      2,
      _QuizCategory.tactics,
      '슈터가 연속된 두 개의 오프볼 스크린을 통과하며 수비 추격을 흔드는 액션은?',
      'What action sends a shooter through two consecutive off-ball screens?',
      '스태거 스크린',
      'stagger screen',
      '스태거는 한 번의 스크린보다 수비 추격 각도와 소통을 더 어렵게 만듭니다.',
      'A stagger makes chasing angles and defensive communication harder than one screen.',
      '오프볼 문항은 볼 없는 선수의 경로와 수비 추격 방향을 보세요.',
      'Off-ball questions should read the cutter path and defender chase angle.',
    ),
    _conceptMcqSeed(
      'pistol_action',
      3,
      _QuizCategory.tactics,
      '사이드에서 핸드오프, 볼스크린, 컷을 빠르게 연결해 초반 우위를 만드는 세트는?',
      'What side action links handoff, ball screen, and cut to create early advantage?',
      '피스톨 액션',
      'pistol action',
      '피스톨은 전환 초반 수비가 자리를 잡기 전에 여러 선택지를 한쪽에 압축합니다.',
      'Pistol compresses multiple options on one side before the defense is set.',
      '세트 오펜스 문항은 첫 패스 뒤 이어지는 두 번째 행동까지 보세요.',
      'Set-offense questions should include the second action after the first pass.',
    ),
    _conceptMcqSeed(
      'horns_alignment',
      2,
      _QuizCategory.positions,
      '두 빅맨이 엘보 근처에 서고 볼 핸들러가 중앙에서 선택지를 여는 공격 배치는?',
      'What alignment places two bigs near the elbows with the ball handler up top?',
      '혼즈 세트',
      'horns alignment',
      '혼즈는 핸드오프, 하이로우, 픽앤롤로 이어질 수 있는 균형 잡힌 시작 배치입니다.',
      'Horns is a balanced start for handoffs, high-low action, or pick-and-roll.',
      '공격 배치는 멈춘 모양보다 이어지는 선택지를 함께 외우세요.',
      'Learn alignments through their follow-up options, not just the shape.',
    ),
    _conceptMcqSeed(
      'delay_offense_big_hub',
      3,
      _QuizCategory.tactics,
      '트레일러 빅맨이 탑에서 공을 잡고 핸드오프와 컷을 연결하는 공격 흐름은?',
      'What offense uses a trailing big at the top as a hub for handoffs and cuts?',
      '딜레이 오펜스',
      'delay offense',
      '딜레이는 속공이 멈춘 뒤에도 빅맨 패싱과 오프볼 움직임으로 리듬을 이어갑니다.',
      'Delay keeps flow after transition stalls through big passing and off-ball movement.',
      '전환 공격은 빠른 슛만이 아니라 세컨더리 액션으로 이어질 수 있습니다.',
      'Transition can flow into secondary action, not only quick shots.',
    ),
    _conceptMcqSeed(
      'zone_overload',
      3,
      _QuizCategory.tactics,
      '지역방어 한쪽에 공격 숫자를 몰아 패스 선택지를 만들고 로테이션을 강요하는 전술은?',
      'What tactic overloads one side of a zone to create passing options and force rotation?',
      '존 오버로드',
      'zone overload',
      '존 오버로드는 수비 구역 사이에 두 명을 세워 한 수비수가 둘을 맡게 만듭니다.',
      'Zone overload puts two threats between zones so one defender must guard two.',
      '지역공격은 빈 공간과 수비수 책임이 겹치는 지점을 찾으세요.',
      'Against zone, find the gap where defender responsibilities overlap.',
    ),
    _conceptMcqSeed(
      'short_corner_zone',
      3,
      _QuizCategory.tactics,
      '지역방어 뒤 베이스라인 근처에서 빅맨이 받아 하이로우와 마무리를 노리는 지점은?',
      'What zone-offense spot behind the defense lets a big catch for high-low or finishes?',
      '숏 코너',
      'short corner',
      '숏 코너는 지역방어의 뒤 공간을 점유해 림 보호 수비를 앞뒤로 흔듭니다.',
      'The short corner occupies the back of a zone and stretches rim help vertically.',
      '지역방어 문항은 외곽 패스보다 뒤 공간 점유를 함께 보세요.',
      'Zone questions should include back-space occupation, not only perimeter passing.',
    ),
    _conceptMcqSeed(
      'skip_pass_window',
      2,
      _QuizCategory.technique,
      '도움 수비가 한쪽으로 몰렸을 때 반대편 슈터에게 길게 넘기는 패스 선택은?',
      'What pass sends the ball across the floor to the weak-side shooter after help loads up?',
      '스킵 패스',
      'skip pass',
      '스킵 패스는 수비 로테이션보다 공이 먼저 도착해야 가치가 생깁니다.',
      'A skip pass works when the ball beats the defensive rotation.',
      '패스 문항은 거리보다 수비가 이미 어디로 기울었는지 보세요.',
      'Passing questions should read where the defense has tilted, not only distance.',
    ),
    _conceptMcqSeed(
      'closeout_angle_force',
      2,
      _QuizCategory.technique,
      '슈터에게 달려가며 가운데 돌파를 막고 원하는 방향으로 몰기 위해 잡는 접근 각도는?',
      'What closeout path takes away the middle and sends the shooter toward the intended side?',
      '클로즈아웃 각도',
      'closeout angle',
      '클로즈아웃은 빠르게 가는 것보다 마지막 두 걸음에서 방향을 제어하는 것이 중요합니다.',
      'A closeout is less about pure speed and more about controlling direction in the last steps.',
      '수비 기술은 접근 속도와 멈추는 균형을 같이 훈련하세요.',
      'Defensive skill trains approach speed and stopping balance together.',
    ),
    _conceptMcqSeed(
      'no_middle_principle',
      2,
      _QuizCategory.tactics,
      '상대 볼 핸들러를 중앙으로 보내지 않고 사이드라인이나 베이스라인 쪽으로 유도하는 원칙은?',
      'What defensive principle denies the middle and sends the ball toward sideline or baseline?',
      '노 미들 수비',
      'no-middle defense',
      '노 미들은 도움 위치를 예측 가능하게 만들고 중앙 페인트 침투를 줄입니다.',
      'No-middle defense makes help predictable and reduces central paint attacks.',
      '팀 수비 문항은 개인 수비 방향과 뒤 도움 위치를 함께 보세요.',
      'Team-defense questions pair on-ball direction with help behind it.',
    ),
    _conceptMcqSeed(
      'transition_cross_match',
      3,
      _QuizCategory.positions,
      '속공 수비에서 원래 매치업이 아니어도 가장 위험한 선수부터 임시로 맡는 원칙은?',
      'What transition principle temporarily guards the most dangerous player even if it is not the original matchup?',
      '크로스 매치',
      'cross match',
      '속공에서는 완벽한 매치업 복귀보다 공, 림, 오픈 슈터를 먼저 지우는 것이 중요합니다.',
      'In transition, stopping ball, rim, and open shooters comes before perfect matchups.',
      '전환 수비는 내 사람을 찾기 전 가장 큰 위험을 먼저 찾으세요.',
      'Transition defense finds the biggest danger before the original matchup.',
    ),
    _conceptMcqSeed(
      'early_drag_screen',
      2,
      _QuizCategory.tactics,
      '전환 초반 빅맨이 따라오며 바로 볼스크린을 걸어 수비 정렬 전에 우위를 만드는 행동은?',
      'What early transition screen by a trailing big attacks before the defense is set?',
      '드래그 스크린',
      'drag screen',
      '드래그 스크린은 수비가 매치업과 도움 위치를 정하기 전에 속도 우위를 씁니다.',
      'A drag screen uses tempo before the defense has matchups and help positions set.',
      '전환 공격 문항은 빠른 슛뿐 아니라 빠른 스크린 타이밍도 보세요.',
      'Transition offense includes early screen timing, not only quick shots.',
    ),
    _conceptMcqSeed(
      'advantage_domino',
      3,
      _QuizCategory.tactics,
      '첫 돌파로 만든 작은 수비 붕괴가 추가 패스와 로테이션을 연쇄적으로 만드는 현상은?',
      'What describes a small first advantage creating chained passes and rotations?',
      '어드밴티지 도미노',
      'advantage domino',
      '좋은 공격은 한 번의 돌파가 끝이 아니라 수비가 계속 늦게 반응하게 만드는 흐름입니다.',
      'Good offense turns one drive into a chain that keeps the defense late.',
      '공격 문항은 첫 우위가 두 번째, 세 번째 선택으로 이어지는지 보세요.',
      'Offensive questions should follow the first advantage into the next choices.',
    ),
    _conceptMcqSeed(
      'extra_pass_read',
      1,
      _QuizCategory.mindset,
      '좋은 슛을 더 좋은 슛으로 바꾸기 위해 한 번 더 패스하는 팀 공격 습관은?',
      'What team habit turns a good shot into a better shot with one more pass?',
      '엑스트라 패스',
      'extra pass',
      '엑스트라 패스는 이타심만이 아니라 수비 로테이션이 완전히 무너진 타이밍을 쓰는 판단입니다.',
      'An extra pass is not only unselfishness; it uses the moment the rotation fully breaks.',
      '슛 선택은 누가 던지느냐보다 얼마나 열린 슛인지 먼저 보세요.',
      'Shot selection starts with openness before who takes it.',
    ),
    _conceptMcqSeed(
      'corner_three_value',
      2,
      _QuizCategory.tactics,
      '거리와 킥아웃 생성 과정을 고려할 때 현대 공격에서 특히 가치가 큰 외곽 슛은?',
      'Which perimeter shot is especially valuable because of distance and kick-out creation?',
      '코너 3점',
      'corner three',
      '코너 3점은 상대 도움 수비를 벌리고 돌파 뒤 높은 기대값의 패스 목표가 됩니다.',
      'The corner three stretches help and becomes a high-value kick-out target after drives.',
      '슛 가치는 위치와 그 슛이 어떻게 만들어졌는지를 같이 봐야 합니다.',
      'Shot value depends on location and how the shot was created.',
    ),
    _conceptMcqSeed(
      'rim_pressure_creation',
      2,
      _QuizCategory.tactics,
      '돌파나 포스트업으로 림 근처 수비를 끌어들여 슛과 패스 선택을 여는 공격 압박은?',
      'What pressure pulls defenders to the rim through drives or post touches to open shots and passes?',
      '림 압박',
      'rim pressure',
      '림 압박이 있어야 수비가 안으로 좁혀지고 외곽 패스와 파울 유도가 살아납니다.',
      'Rim pressure collapses the defense, enabling kick-outs and fouls.',
      '공격 문항은 외곽 슛 전에 수비를 안으로 끌어들였는지 보세요.',
      'Offense questions should ask whether the defense was pulled inside before the outside shot.',
    ),
    _conceptMcqSeed(
      'free_throw_routine',
      1,
      _QuizCategory.mindset,
      '자유투 전 호흡, 드리블 수, 시선 고정을 반복해 압박을 줄이는 절차는?',
      'What repeated breath, dribble count, and gaze process reduces pressure before a free throw?',
      '자유투 루틴',
      'free-throw routine',
      '루틴은 성공을 보장하지 않지만 매번 같은 실행 단서로 주의를 되돌립니다.',
      'A routine does not guarantee makes, but returns attention to consistent execution cues.',
      '마인드 문항은 감정보다 반복 가능한 행동 순서를 찾으세요.',
      'Mindset questions should find repeatable action order rather than emotion.',
    ),
    _conceptMcqSeed(
      'ato_execution',
      2,
      _QuizCategory.tactics,
      '타임아웃 직후 코치가 설계한 첫 공격을 정확히 수행하는 상황은?',
      'What describes executing the first designed possession after a timeout?',
      'ATO 실행',
      'ATO execution',
      'ATO는 전술판 그림보다 선수들이 타이밍과 간격을 정확히 맞출 때 가치가 납니다.',
      'ATO value comes when players match timing and spacing, not only when the diagram looks good.',
      '작전 문항은 첫 옵션이 막혔을 때 두 번째 옵션까지 확인하세요.',
      'Set-play questions should include the second option if the first is covered.',
    ),
    _conceptMcqSeed(
      'late_clock_spacing',
      2,
      _QuizCategory.tactics,
      '샷클락이 낮을 때 볼 핸들러의 돌파 각과 킥아웃을 살리기 위해 동료들이 넓게 서는 원칙은?',
      'What principle keeps teammates spaced wide late in the clock to preserve drive and kick angles?',
      '레이트클락 스페이싱',
      'late-clock spacing',
      '시간이 적을수록 모두 공으로 몰리면 수비가 좁아져 마지막 선택이 더 어려워집니다.',
      'When time is low, crowding the ball lets the defense shrink and makes the final choice harder.',
      '클락 문항은 빠른 슛과 좋은 공간 중 무엇을 지켜야 하는지 묻습니다.',
      'Clock questions ask how to keep space while acting quickly.',
    ),
    _conceptMcqSeed(
      'box_and_one',
      3,
      _QuizCategory.tactics,
      '네 명은 지역처럼 서고 한 명은 핵심 득점원을 전담으로 따라가는 혼합 수비는?',
      'What hybrid defense has four players zone up while one face-guards the main scorer?',
      '박스 앤 원',
      'box-and-one',
      '박스 앤 원은 한 선수 의존도가 큰 공격을 흔들지만 다른 선수의 판단과 슈팅을 시험합니다.',
      'Box-and-one disrupts star-heavy offense but tests the other players’ decisions and shooting.',
      '특수 수비는 막는 선수와 열어 주는 약점을 같이 보세요.',
      'Special defenses should be read through both target and weakness.',
    ),
    _conceptMcqSeed(
      'two_three_zone_gap',
      2,
      _QuizCategory.tactics,
      '2-3 지역방어를 공략할 때 수비수 둘 사이 빈 지점에 공을 넣는 원칙은?',
      'What principle attacks a 2-3 zone by putting the ball between two defenders?',
      '존 갭 공략',
      'zone gap attack',
      '지역방어는 사람보다 구역을 막기 때문에 구역 사이에 공이 들어가면 책임이 겹칩니다.',
      'Because zone guards areas, the ball in a gap makes responsibilities overlap.',
      '지역공격은 패스 횟수보다 어느 구역 사이를 찔렀는지 보세요.',
      'Zone offense should be judged by which gap is attacked, not pass count.',
    ),
    _conceptMcqSeed(
      'hit_find_get_rebound',
      1,
      _QuizCategory.technique,
      '슛이 올라간 뒤 상대를 먼저 몸으로 막고 공을 찾고 잡는 리바운드 순서는?',
      'What rebound sequence hits the opponent first, then finds and gets the ball?',
      '히트-파인드-겟',
      'hit-find-get',
      '리바운드는 점프력만이 아니라 먼저 상대 진입로를 지우는 위치 싸움입니다.',
      'Rebounding is positioning to erase the opponent’s path before it is jumping.',
      '리바운드 문항은 공만 보지 말고 먼저 맡을 선수를 찾으세요.',
      'Rebound questions should find the opponent before only watching the ball.',
    ),
    _conceptMcqSeed(
      'verticality_rule',
      2,
      _QuizCategory.rules,
      '수비수가 자기 수직 공간을 지키며 접촉을 최소화해 림을 방어하는 원칙은?',
      'What principle lets a defender protect the rim by staying within their vertical space?',
      '버티컬리티',
      'verticality',
      '버티컬리티는 팔을 앞으로 내리찍는 블록보다 자기 공간을 위로 지키는 수비입니다.',
      'Verticality protects space upward rather than swiping forward for a block.',
      '림 수비 문항은 점프 높이보다 몸통과 팔의 방향을 보세요.',
      'Rim-defense questions should inspect torso and arm direction more than jump height.',
    ),
    _conceptMcqSeed(
      'take_foul_rule_flow',
      2,
      _QuizCategory.rules,
      '전환 공격을 고의 파울로 끊는 행위에 불이익을 줘 속공 흐름을 보호하려는 규칙 관점은?',
      'What rule lens penalizes intentional transition-stopping fouls to protect fast-break flow?',
      '테이크 파울 억제',
      'take-foul deterrence',
      '테이크 파울 규칙은 벌칙 암기가 아니라 리그가 전환 공격 가치를 지키려는 방향과 연결됩니다.',
      'Take-foul rules connect to protecting transition value, not only memorizing penalties.',
      '규칙 변화는 어떤 플레이 스타일을 장려하는지 함께 보세요.',
      'Rule changes should be tied to the style of play they encourage.',
    ),
    _conceptMcqSeed(
      'back_to_back_recovery',
      1,
      _QuizCategory.training,
      '연속 경기 사이 수면, 수분, 가벼운 움직임을 우선해 다음 경기 품질을 지키는 계획은?',
      'What plan prioritizes sleep, hydration, and light movement between consecutive games?',
      '백투백 회복 루틴',
      'back-to-back recovery routine',
      '백투백 회복은 특별한 장비보다 반복 가능한 기본 루틴이 다음 경기 집중을 지킵니다.',
      'Back-to-back recovery relies first on repeatable basics that protect next-game focus.',
      '회복 문항은 오늘 피로가 내일 첫 쿼터에 미치는 영향을 보세요.',
      'Recovery questions should connect today’s fatigue to tomorrow’s first quarter.',
    ),
    _conceptMcqSeed(
      'timeout_hydration_micro',
      1,
      _QuizCategory.nutrition,
      '쿼터 사이와 타임아웃마다 작은 양의 수분을 반복 보충해 후반 집중을 지키는 습관은?',
      'What habit uses small repeated fluid intake during timeouts and quarter breaks to protect late focus?',
      '마이크로 수분 보충',
      'micro hydration',
      '농구는 교체와 타임아웃이 있어 작은 수분 루틴을 반복하기 좋은 종목입니다.',
      'Basketball has substitutions and timeouts that make small hydration routines practical.',
      '영양 문항은 경기 구조 안에서 언제 실행할 수 있는지 보세요.',
      'Nutrition questions should ask when the habit fits the sport structure.',
    ),
    _conceptMcqSeed(
      'next_play_mindset',
      1,
      _QuizCategory.mindset,
      '실책 직후 판정이나 이전 장면에 머무르지 않고 다음 수비 위치로 돌아가는 태도는?',
      'What mindset returns to the next defensive spot instead of staying with the last mistake or call?',
      '넥스트 플레이 마인드셋',
      'next-play mindset',
      '농구는 전환이 빨라 실수 분석보다 먼저 다음 위치로 돌아가는 행동이 필요합니다.',
      'Basketball changes ends quickly, so the first response is getting to the next spot.',
      '마인드 문항은 생각을 멈추는 것이 아니라 다음 행동을 시작하는지 보세요.',
      'Mindset questions should check whether the next action starts.',
    ),
    _conceptMcqSeed(
      'load_management_minutes',
      2,
      _QuizCategory.training,
      '시즌 중 선수의 출전 시간, 이동 피로, 회복 상태를 함께 조절하는 관리 방식은?',
      'What management approach balances minutes, travel fatigue, and recovery during a season?',
      '출전 부하 관리',
      'minutes load management',
      '부하 관리는 쉬게 하는 핑계가 아니라 중요한 경기에서 수행 품질을 지키는 계획입니다.',
      'Load management is a plan to protect performance quality, not merely an excuse to rest.',
      '훈련 문항은 한 경기 기록보다 시즌 누적 피로를 함께 보세요.',
      'Training questions should include season fatigue, not only one-game stats.',
    ),
    _conceptMcqSeed(
      'advantage_stop_ball',
      1,
      _QuizCategory.positions,
      '속공을 맞았을 때 가장 가까운 수비수가 먼저 볼을 늦추는 기본 책임은?',
      'What transition responsibility has the nearest defender slow the ball first?',
      '스톱 더 볼',
      'stop the ball',
      '속공 수비는 완벽한 매치업보다 볼의 속도를 줄여 뒤 수비가 자리 잡을 시간을 버는 일부터 시작합니다.',
      'Transition defense starts by slowing the ball so trailing defenders can organize.',
      '전환 수비 문항은 누가 공을 멈추고 누가 림을 지키는지 나눠 보세요.',
      'Transition questions should split who stops ball and who protects the rim.',
    ),
    _conceptMcqSeed(
      'paint_touch_kickout',
      2,
      _QuizCategory.tactics,
      '페인트존 터치로 수비를 좁힌 뒤 외곽으로 내주는 공격 흐름은?',
      'What offensive flow touches the paint to collapse defense before kicking out?',
      '페인트 터치 킥아웃',
      'paint-touch kick-out',
      '페인트 터치가 있으면 외곽 슛은 단순 패스보다 수비 붕괴 뒤 나온 좋은 선택이 됩니다.',
      'A paint touch turns the outside shot into a choice created by defensive collapse.',
      '외곽슛 문항은 슛 전 수비가 안으로 모였는지 확인하세요.',
      'Perimeter-shot questions should check whether the defense collapsed first.',
    ),
    _conceptMcqSeed(
      'post_split_cut',
      3,
      _QuizCategory.tactics,
      '포스트에 공이 들어간 뒤 주변 가드들이 스크린과 컷으로 수비 시선을 흔드는 액션은?',
      'What action uses screens and cuts around a post touch to move defenders’ eyes?',
      '포스트 스플릿',
      'post split',
      '포스트업은 1대1만이 아니라 주변 컷과 슈터 움직임을 여는 패스 허브가 될 수 있습니다.',
      'A post touch can be a passing hub for cuts and shooters, not only isolation.',
      '포스트 문항은 등지고 공격하는 선수와 주변 움직임을 같이 보세요.',
      'Post questions should include both the post player and surrounding movement.',
    ),
    _conceptMcqSeed(
      'defensive_rebound_outlet',
      1,
      _QuizCategory.technique,
      '수비 리바운드 후 안전한 첫 패스로 속공 시작점을 만드는 행동은?',
      'What action uses a safe first pass after a defensive rebound to start transition?',
      '아울렛 패스',
      'outlet pass',
      '아울렛 패스는 리바운드 성공을 공격 전환으로 바꾸는 첫 연결입니다.',
      'An outlet pass turns a rebound into the first link of transition offense.',
      '리바운드 문항은 잡은 뒤 첫 패스 방향까지 연결하세요.',
      'Rebound questions should connect the catch to the first pass direction.',
    ),
  ];
}

List<_ConceptMcqSeed> _tennisHundredFloorConceptSeeds() {
  return <_ConceptMcqSeed>[
    _conceptMcqSeed(
      'deep_central_return',
      2,
      _QuizCategory.tactics,
      '강한 첫 서브를 받은 뒤 서버의 첫 공격 각도를 줄이기 위해 깊고 중앙으로 넣는 리턴은?',
      'What return goes deep and central after a strong first serve to reduce the server’s first-strike angle?',
      '딥 센터 리턴',
      'deep central return',
      '좋은 리턴은 바로 위너를 노리기보다 서버의 다음 공격 각도를 줄이는 역할을 할 수 있습니다.',
      'A good return can neutralize the next attack instead of trying to be a winner.',
      '리턴 문항은 득점보다 다음 공의 위험을 얼마나 낮췄는지 보세요.',
      'Return questions should ask how much the next-ball danger is reduced.',
    ),
    _conceptMcqSeed(
      'serve_plus_one_wide',
      3,
      _QuizCategory.tactics,
      '와이드 서브로 코트를 열고 다음 포핸드로 빈 공간을 공격하는 패턴은?',
      'What pattern opens the court with a wide serve and attacks space with the next forehand?',
      '와이드 서브 플러스 원',
      'wide serve plus one',
      '서브 플러스 원은 서브 하나가 아니라 다음 공을 미리 설계하는 공격 패턴입니다.',
      'Serve plus one is a planned first-strike pattern, not only the serve itself.',
      '서브 문항은 착지 지점과 다음 샷 코스를 함께 보세요.',
      'Serve questions should pair landing target with the next-shot direction.',
    ),
    _conceptMcqSeed(
      'body_serve_jam',
      2,
      _QuizCategory.tactics,
      '리턴어의 팔 공간을 줄여 큰 스윙을 못 하게 몸쪽으로 붙이는 서브 선택은?',
      'What serve target jams the returner by reducing arm space for a full swing?',
      '바디 서브',
      'body serve',
      '바디 서브는 에이스보다 리턴 품질을 낮춰 다음 공을 쉽게 만들려는 선택입니다.',
      'A body serve often aims to lower return quality rather than hit an ace.',
      '서브 타깃은 코스뿐 아니라 상대 스윙 공간을 기준으로도 판단하세요.',
      'Serve targets can be judged by the opponent’s swing space, not only court location.',
    ),
    _conceptMcqSeed(
      'kick_serve_height',
      2,
      _QuizCategory.technique,
      '높은 바운드로 리턴 타점을 어깨 근처까지 올려 공격을 어렵게 하는 서브는?',
      'What serve uses high bounce to push the return contact point near shoulder height?',
      '킥 서브',
      'kick serve',
      '킥 서브는 속도보다 회전과 바운드 높이로 리턴어의 편한 타점을 빼앗습니다.',
      'A kick serve uses spin and bounce height more than speed to disturb contact.',
      '서브 기술 문항은 공이 튄 뒤 상대 타점을 함께 보세요.',
      'Serve-skill questions should include the opponent’s contact point after bounce.',
    ),
    _conceptMcqSeed(
      'slice_serve_deuce_court',
      2,
      _QuizCategory.technique,
      '듀스 코트에서 바깥으로 휘어 상대를 코트 밖으로 밀어낼 수 있는 회전 서브는?',
      'What spinning serve can curve outward in the deuce court and pull the returner off court?',
      '슬라이스 서브',
      'slice serve',
      '슬라이스 서브는 각도를 열어 다음 샷을 빈 코트로 치기 쉽게 만듭니다.',
      'A slice serve opens angle and can make the next shot easier into space.',
      '서브 코스는 단독 득점보다 다음 코트가 어디 열리는지 보세요.',
      'Serve placement should be read by which court opens next.',
    ),
    _conceptMcqSeed(
      'second_serve_margin',
      1,
      _QuizCategory.mindset,
      '두 번째 서브에서 속도를 줄이고 회전과 높이로 더블 폴트를 줄이는 선택은?',
      'What second-serve choice lowers speed and adds spin and height to reduce double faults?',
      '세컨드 서브 마진',
      'second-serve margin',
      '두 번째 서브는 강함보다 네트 통과 높이와 회전으로 안정성을 확보하는 것이 먼저입니다.',
      'The second serve should first secure net clearance and spin rather than raw pace.',
      '압박 점수에서는 강한 선택과 확률 높은 선택을 구분하세요.',
      'On pressure points, separate aggressive choices from high-percentage choices.',
    ),
    _conceptMcqSeed(
      'crosscourt_geometry',
      2,
      _QuizCategory.tactics,
      '대각선 코트 길이와 낮은 네트 중앙을 이용해 안정적으로 랠리를 이어가는 선택은?',
      'What rally choice uses diagonal court length and the lower middle of the net?',
      '크로스코트 기하',
      'crosscourt geometry',
      '크로스코트는 감각적 안전함이 아니라 코트 구조상 더 긴 거리와 낮은 네트를 활용합니다.',
      'Crosscourt safety comes from court geometry: more distance and a lower net path.',
      '샷 선택 문항은 왜 그 방향이 확률 높은지 설명할 수 있어야 합니다.',
      'Shot-choice questions should explain why a direction is higher percentage.',
    ),
    _conceptMcqSeed(
      'down_line_timing',
      3,
      _QuizCategory.tactics,
      '상대가 크로스코트 회복에 치우쳤을 때 라인 쪽으로 방향을 바꾸는 공격 타이밍은?',
      'What attacking timing changes direction down the line when the opponent over-recovers crosscourt?',
      '다운더라인 전환',
      'down-the-line change',
      '다운더라인은 네트가 높고 코트가 짧아 준비 없이 치면 위험하지만 상대 균형이 무너질 때 가치가 큽니다.',
      'Down the line is risky without setup, but valuable when the opponent is off balance.',
      '방향 전환 문항은 공의 높이와 상대 회복 위치를 같이 보세요.',
      'Direction-change questions should read ball height and opponent recovery.',
    ),
    _conceptMcqSeed(
      'inside_out_forehand_pattern',
      2,
      _QuizCategory.tactics,
      '백핸드 코트로 돌아서 상대 백핸드 쪽 대각으로 주도권을 잡는 포핸드 패턴은?',
      'What forehand pattern runs around the backhand side to attack crosscourt toward the opponent’s backhand?',
      '인사이드 아웃 포핸드',
      'inside-out forehand',
      '인사이드 아웃은 코트 위치를 포기하는 만큼 다음 열린 공간까지 계산해야 합니다.',
      'Inside-out forehands give up court position, so the next open court must be planned.',
      '패턴 문항은 한 샷 뒤 회복 위치를 반드시 포함하세요.',
      'Pattern questions should include recovery position after the shot.',
    ),
    _conceptMcqSeed(
      'inside_in_forehand',
      3,
      _QuizCategory.tactics,
      '인사이드 아웃을 예상한 상대를 역이용해 같은 준비에서 라인으로 치는 포핸드는?',
      'What forehand uses the same run-around preparation but attacks down the line instead of crosscourt?',
      '인사이드 인 포핸드',
      'inside-in forehand',
      '인사이드 인은 상대 예측을 흔들 수 있지만 라인 아웃 위험이 커서 균형이 중요합니다.',
      'Inside-in can punish anticipation, but line risk makes balance important.',
      '공격 전환은 상대 예측과 자신의 몸 균형을 함께 봐야 합니다.',
      'Attacking changes should read opponent anticipation and your body balance.',
    ),
    _conceptMcqSeed(
      'neutral_ball_tolerance',
      1,
      _QuizCategory.mindset,
      '공격 기회가 아닌 중립 볼에서 무리하지 않고 깊이와 높이로 랠리를 유지하는 능력은?',
      'What ability keeps depth and height on neutral balls instead of forcing attack?',
      '중립볼 인내',
      'neutral-ball tolerance',
      '좋은 선수는 공격할 공과 기다릴 공을 구분해 언포스드 에러를 줄입니다.',
      'Good players reduce unforced errors by separating attack balls from patience balls.',
      '마인드 문항은 참는 것이 소극성이 아니라 선택 기준임을 보세요.',
      'Mindset questions should frame patience as a selection standard, not passivity.',
    ),
    _conceptMcqSeed(
      'approach_shot_depth',
      2,
      _QuizCategory.positions,
      '네트로 전진하기 전 상대를 뒤로 밀어 발리 준비 시간을 버는 깊은 샷은?',
      'What deep shot pushes the opponent back before moving forward to volley?',
      '어프로치 샷 깊이',
      'approach-shot depth',
      '어프로치 샷은 세게 치기보다 상대가 낮고 어려운 패스를 치게 만드는 깊이가 중요합니다.',
      'An approach shot values depth that forces a difficult pass more than pure pace.',
      '네트 전진 문항은 전진 전 샷이 상대를 얼마나 밀었는지 확인하세요.',
      'Net-approach questions should check how much the approach pushed the opponent.',
    ),
    _conceptMcqSeed(
      'first_volley_target',
      2,
      _QuizCategory.technique,
      '네트에 붙은 뒤 첫 발리를 빈 공간보다 안전하게 깊은 코스로 보내 흐름을 유지하는 선택은?',
      'What first-volley choice keeps control by sending the ball deep instead of forcing a tiny open spot?',
      '첫 발리 타깃',
      'first-volley target',
      '첫 발리는 화려한 위너보다 상대 패스 각도를 줄이는 깊은 타깃이 안정적입니다.',
      'The first volley is often safer deep, reducing passing angles instead of forcing a winner.',
      '발리 문항은 손기술과 함께 다음 패스 각을 어떻게 줄이는지 보세요.',
      'Volley questions should read how the target reduces the next passing angle.',
    ),
    _conceptMcqSeed(
      'defensive_lob_choice',
      2,
      _QuizCategory.tactics,
      '상대가 네트에 너무 가까울 때 머리 위로 깊게 넘겨 위치를 되돌리는 수비 선택은?',
      'What defensive choice sends the ball high and deep over a net player to reset position?',
      '수비 로브',
      'defensive lob',
      '수비 로브는 단순 버티기가 아니라 상대 전진 위치를 뒤로 밀어 시간을 되찾는 선택입니다.',
      'A defensive lob is not only survival; it buys time by pushing the net player back.',
      '수비 샷은 득점보다 위치 회복 시간을 만드는지 보세요.',
      'Defensive-shot questions should ask whether the shot creates recovery time.',
    ),
    _conceptMcqSeed(
      'drop_shot_disguise',
      3,
      _QuizCategory.technique,
      '긴 스윙처럼 보이게 준비하다 마지막에 짧게 떨어뜨려 상대 전진을 늦추는 기술은?',
      'What skill prepares like a deeper shot then softly drops the ball short to delay opponent movement?',
      '드롭샷 위장',
      'drop-shot disguise',
      '좋은 드롭샷은 짧게 치는 감각보다 직전까지 같은 준비 동작을 유지하는 데서 나옵니다.',
      'A good drop shot comes from holding the same preparation, not only soft touch.',
      '기술 문항은 임팩트보다 상대가 언제 알아차렸는지 함께 보세요.',
      'Technique questions should ask when the opponent recognizes the shot.',
    ),
    _conceptMcqSeed(
      'slice_backhand_reset',
      2,
      _QuizCategory.technique,
      '수비에서 낮게 깔아 상대 타점을 낮추고 랠리 템포를 다시 정리하는 백핸드 샷은?',
      'What backhand shot stays low to lower the opponent’s contact and reset rally tempo?',
      '슬라이스 백핸드 리셋',
      'slice backhand reset',
      '슬라이스는 약한 샷이 아니라 높이와 시간을 조절해 다음 위치를 회복하는 기술입니다.',
      'Slice is not merely weak; it controls height and time for recovery.',
      '샷 품질은 속도뿐 아니라 상대 타점을 어디로 만드는지로 보세요.',
      'Shot quality includes the opponent’s contact point, not only speed.',
    ),
    _conceptMcqSeed(
      'moonball_tempo_change',
      1,
      _QuizCategory.tactics,
      '높고 깊은 공으로 상대 리듬을 늦추고 실수를 유도하는 랠리 변화는?',
      'What rally change uses high deep balls to slow rhythm and invite errors?',
      '문볼 템포 변화',
      'moonball tempo change',
      '높은 깊이는 공격처럼 보이지 않아도 상대가 편한 타점에서 치지 못하게 만듭니다.',
      'High depth may not look aggressive, but it denies comfortable contact.',
      '템포 문항은 빠르게 치는 선택과 느리게 흔드는 선택을 구분하세요.',
      'Tempo questions should separate playing faster from disrupting with slower shape.',
    ),
    _conceptMcqSeed(
      'serve_landing_recovery',
      2,
      _QuizCategory.positions,
      '서브 후 착지와 첫 스텝으로 다음 리턴 코스에 대응할 위치를 빠르게 회복하는 능력은?',
      'What ability uses landing and first step after serving to recover for the return direction?',
      '서브 후 회복 스텝',
      'serve recovery step',
      '서브 동작은 임팩트에서 끝나지 않고 다음 공을 받을 위치로 이어져야 합니다.',
      'The serve does not end at contact; it must flow into position for the next ball.',
      '서브 문항은 넣는 동작과 다음 수비 위치를 연결하세요.',
      'Serve questions should connect delivery with the next defensive position.',
    ),
    _conceptMcqSeed(
      'split_step_contact_timing',
      1,
      _QuizCategory.technique,
      '상대가 공을 맞히는 순간 가볍게 착지해 어느 방향이든 출발할 준비를 하는 동작은?',
      'What movement lands lightly as the opponent contacts the ball so the player can move either way?',
      '스플릿 스텝 타이밍',
      'split-step timing',
      '스플릿 스텝은 점프 자체보다 상대 임팩트와 착지 타이밍이 핵심입니다.',
      'The split step is about landing timing with opponent contact, not the jump itself.',
      '풋워크 문항은 움직인 양보다 출발 준비 순간을 보세요.',
      'Footwork questions should inspect the ready moment more than movement volume.',
    ),
    _conceptMcqSeed(
      'recover_to_center',
      1,
      _QuizCategory.positions,
      '샷을 친 뒤 상대가 칠 수 있는 각의 가운데로 돌아가는 기본 위치 회복은?',
      'What basic recovery returns to the middle of the opponent’s possible angles after a shot?',
      '센터 회복',
      'recovery to center',
      '센터 회복은 코트 정중앙이 아니라 상대가 만들 수 있는 각도의 중심으로 돌아가는 판단입니다.',
      'Recovery center is the middle of the opponent’s likely angles, not always the geometric center.',
      '위치 문항은 공 위치와 상대 선택지를 함께 계산하세요.',
      'Positioning questions should combine ball location with opponent options.',
    ),
    _conceptMcqSeed(
      'open_stance_defense',
      2,
      _QuizCategory.technique,
      '넓은 공을 따라가며 몸을 완전히 닫지 않고 회복 시간을 확보하는 수비 타격 자세는?',
      'What defensive hitting stance reaches wide balls without fully closing the body, preserving recovery time?',
      '오픈 스탠스 수비',
      'open-stance defense',
      '오픈 스탠스는 급한 상황에서 회전과 회복을 빠르게 연결하는 실전 선택이 될 수 있습니다.',
      'Open stance can link rotation and recovery quickly in pressured positions.',
      '자세 문항은 이상적인 폼보다 공까지 도달한 상황을 먼저 보세요.',
      'Stance questions should read the ball-reach situation before ideal form.',
    ),
    _conceptMcqSeed(
      'closed_stance_drive',
      2,
      _QuizCategory.technique,
      '시간이 충분할 때 앞발을 더 닫아 체중 이동과 직선 타구를 만들기 쉬운 자세는?',
      'What stance can use a more closed front foot to create weight transfer and a driving ball when time allows?',
      '클로즈드 스탠스 드라이브',
      'closed-stance drive',
      '닫힌 자세는 준비 시간이 있을 때 전진 체중 이동과 방향 제어에 도움을 줄 수 있습니다.',
      'A closed stance can help forward weight transfer and directional control when time is available.',
      '스탠스는 좋고 나쁨보다 시간과 공 위치에 맞는지로 판단하세요.',
      'Judge stance by time and ball location rather than good or bad labels.',
    ),
    _conceptMcqSeed(
      'clay_patience_pattern',
      2,
      _QuizCategory.tactics,
      '클레이에서 긴 랠리와 높은 바운드를 감안해 무리한 위너보다 반복 깊이를 선택하는 운영은?',
      'What clay-court approach chooses repeated depth over forced winners because of long rallies and high bounce?',
      '클레이 인내 패턴',
      'clay patience pattern',
      '클레이는 포인트가 길어지기 쉬워 준비 없는 공격보다 누적 압박이 중요합니다.',
      'Clay often lengthens points, so accumulated pressure matters more than unprepared attacks.',
      '코트 표면 문항은 바운드와 포인트 길이가 선택을 어떻게 바꾸는지 보세요.',
      'Surface questions should connect bounce and point length to shot choice.',
    ),
    _conceptMcqSeed(
      'grass_first_strike',
      3,
      _QuizCategory.tactics,
      '잔디에서 낮고 빠른 바운드를 활용해 서브와 첫 공격으로 짧은 포인트를 노리는 운영은?',
      'What grass-court approach uses low fast bounce to seek short points with serve and first strike?',
      '잔디 퍼스트 스트라이크',
      'grass first-strike pattern',
      '잔디는 낮은 바운드 때문에 빠른 준비와 전진 선택의 가치가 커질 수 있습니다.',
      'Grass can reward quick preparation and forward choices because of lower bounce.',
      '표면 문항은 같은 샷도 코트에 따라 왜 달라지는지 묻습니다.',
      'Surface questions ask why the same shot changes by court type.',
    ),
    _conceptMcqSeed(
      'tiebreak_risk_control',
      3,
      _QuizCategory.mindset,
      '타이브레이크에서 모든 공을 위너로 끝내기보다 확률 높은 패턴과 리스크를 조절하는 태도는?',
      'What mindset in a tiebreak controls risk through high-percentage patterns rather than forcing every winner?',
      '타이브레이크 리스크 관리',
      'tiebreak risk control',
      '압박 점수에서는 과감함과 무리함을 구분하고 자신 있는 패턴을 반복하는 것이 중요합니다.',
      'Pressure points require separating aggression from low-percentage forcing.',
      '마인드 문항은 점수 의미보다 다음 선택의 확률을 묻습니다.',
      'Mindset questions should ask about the probability of the next choice.',
    ),
    _conceptMcqSeed(
      'break_point_routine',
      2,
      _QuizCategory.mindset,
      '브레이크 포인트에서 점수 의미보다 첫 행동 계획으로 주의를 돌리는 절차는?',
      'What process shifts attention on break point from score meaning to the first action plan?',
      '브레이크 포인트 루틴',
      'break-point routine',
      '루틴은 긴장을 없애는 마법이 아니라 리턴 위치와 첫 스윙 목표를 분명하게 합니다.',
      'A routine does not erase nerves; it clarifies return position and first-swing target.',
      '압박 문항은 감정 상태보다 실행 단서를 확인하세요.',
      'Pressure questions should check execution cues more than emotional state.',
    ),
    _conceptMcqSeed(
      'error_recovery_pattern',
      1,
      _QuizCategory.mindset,
      '쉬운 실수 뒤 다음 포인트 첫 두 타구를 안정적으로 넣어 흐름을 되찾는 방법은?',
      'What pattern restores rhythm after an easy miss by making the next two shots safely?',
      '실수 후 안정 패턴',
      'post-error safety pattern',
      '실수 후 바로 더 세게 치면 같은 실수가 반복될 수 있어 먼저 랠리 기준을 회복해야 합니다.',
      'Hitting harder right after an error can repeat the mistake, so first restore rally standards.',
      '실수 대응은 후회가 아니라 다음 두 타구 계획으로 바꾸세요.',
      'Error response should become a next-two-shots plan, not regret.',
    ),
    _conceptMcqSeed(
      'changeover_hydration_plan',
      1,
      _QuizCategory.nutrition,
      '체인지오버마다 갈증 여부와 상관없이 작은 수분과 필요 시 탄수화물을 확인하는 루틴은?',
      'What changeover routine checks small fluid intake and carbohydrate needs regardless of thirst?',
      '체인지오버 보충 루틴',
      'changeover fueling routine',
      '테니스는 휴식 간격이 정해져 있어 더위와 긴 경기에서 보충 루틴을 만들기 좋습니다.',
      'Tennis has set breaks that make fueling routines practical in heat and long matches.',
      '영양 문항은 경기 구조 안에서 실행 타이밍을 찾으세요.',
      'Nutrition questions should find the timing built into the sport.',
    ),
    _conceptMcqSeed(
      'heat_cramp_prevention',
      2,
      _QuizCategory.nutrition,
      '더운 날 긴 경기에서 수분, 전해질, 페이스 조절로 후반 근육 경련 위험을 낮추는 계획은?',
      'What plan lowers late cramp risk in hot long matches through fluids, electrolytes, and pacing?',
      '더위 경련 예방',
      'heat-cramp prevention',
      '경련 예방은 한 가지 음료보다 땀 손실, 전해질, 경기 강도 조절을 함께 보는 문제입니다.',
      'Cramp prevention involves sweat loss, electrolytes, and intensity control, not one drink alone.',
      '회복 문항은 날씨와 땀 양을 반드시 함께 보세요.',
      'Recovery questions should include weather and sweat rate.',
    ),
    _conceptMcqSeed(
      'doubles_poach_timing',
      2,
      _QuizCategory.tactics,
      '복식에서 네트 플레이어가 상대 크로스 리턴을 예상해 중앙으로 가로채는 움직임은?',
      'What doubles move has the net player cut across to intercept an expected crosscourt return?',
      '포칭 타이밍',
      'poaching timing',
      '포칭은 용기만이 아니라 서버 코스, 리턴 습관, 파트너 커버가 맞아야 합니다.',
      'Poaching needs serve target, return tendency, and partner cover, not courage alone.',
      '복식 문항은 한 명의 움직임이 파트너 위치를 어떻게 바꾸는지 보세요.',
      'Doubles questions should read how one move changes partner coverage.',
    ),
    _conceptMcqSeed(
      'australian_formation',
      3,
      _QuizCategory.positions,
      '복식에서 서버 파트너가 중앙 가까이 서서 리턴 방향을 흔드는 특수 대형은?',
      'What doubles formation places the server’s partner near the middle to disrupt return direction?',
      '오스트레일리안 포메이션',
      'Australian formation',
      '오스트레일리안 포메이션은 리턴어에게 익숙한 크로스 리턴 각을 불편하게 만듭니다.',
      'Australian formation makes the returner’s usual crosscourt angle uncomfortable.',
      '복식 대형은 위치보다 리턴 선택지를 어떻게 줄이는지 보세요.',
      'Doubles formations should be read by how they reduce return options.',
    ),
    _conceptMcqSeed(
      'i_formation_call',
      3,
      _QuizCategory.positions,
      '복식에서 네트 플레이어가 중앙 낮게 앉고 사인으로 이동 방향을 정하는 대형은?',
      'What doubles formation has the net player crouch near the center and use signals for movement direction?',
      '아이 포메이션',
      'I-formation',
      '아이 포메이션은 사전 사인과 서버 코스가 맞아야 혼란이 아니라 압박이 됩니다.',
      'I-formation needs signals and serve location to create pressure rather than confusion.',
      '전술 대형 문항은 사인과 첫 이동 방향까지 포함하세요.',
      'Formation questions should include signals and first movement direction.',
    ),
    _conceptMcqSeed(
      'return_stack_doubles',
      2,
      _QuizCategory.positions,
      '복식에서 리턴팀 두 선수가 같은 쪽에 서서 특정 서버나 코스를 압박하는 배치는?',
      'What doubles return setup puts both receivers on one side to pressure a server or target?',
      '리턴 스택',
      'return stack',
      '리턴 스택은 상대 서브 패턴을 흔들 수 있지만 빈 공간 커버 약속이 필요합니다.',
      'A return stack can disrupt serve patterns but needs clear empty-court coverage rules.',
      '복식 문항은 대형의 장점과 열리는 공간을 함께 보세요.',
      'Doubles questions should include both the formation benefit and exposed space.',
    ),
    _conceptMcqSeed(
      'no_ad_point_choice',
      2,
      _QuizCategory.rules,
      '노애드 경기에서 듀스 후 한 포인트로 게임이 결정될 때 리시버가 코트를 선택하는 상황은?',
      'In no-ad scoring, what situation lets the receiver choose the side for the deciding point after deuce?',
      '노애드 결정 포인트',
      'no-ad deciding point',
      '노애드는 듀스 반복을 줄이고 한 포인트의 리턴 위치와 루틴 가치를 크게 만듭니다.',
      'No-ad scoring reduces repeated deuces and raises the value of return side and routine.',
      '규칙 문항은 점수 방식이 전술 선택을 어떻게 바꾸는지 보세요.',
      'Rules questions should ask how scoring format changes tactics.',
    ),
    _conceptMcqSeed(
      'line_call_reset',
      1,
      _QuizCategory.mindset,
      '애매한 라인 판정 뒤 논쟁에 머무르지 않고 다음 포인트 준비로 돌아가는 루틴은?',
      'What routine returns focus to the next point after a close line call instead of staying in argument?',
      '라인콜 리셋',
      'line-call reset',
      '판정 스트레스는 바로 없애기 어렵지만 다음 포인트 루틴으로 행동을 되돌릴 수 있습니다.',
      'Call stress may not disappear immediately, but routine can return behavior to the next point.',
      '마인드 문항은 억울함보다 다음 준비 행동을 찾으세요.',
      'Mindset questions should find the next preparation action instead of the grievance.',
    ),
    _conceptMcqSeed(
      'unforced_error_tracking',
      1,
      _QuizCategory.fun,
      '상대 압박이 크지 않은 상황에서 스스로 낸 실수를 따로 기록해 안정성을 보는 지표는?',
      'What stat tracks mistakes made without heavy opponent pressure to evaluate stability?',
      '언포스드 에러 추적',
      'unforced-error tracking',
      '언포스드 에러는 단순 못함이 아니라 어떤 공에서 선택 기준이 흔들렸는지 보는 단서입니다.',
      'Unforced errors show where selection standards broke down, not only poor play.',
      '기록 문항은 숫자 뒤의 반복 패턴을 함께 보세요.',
      'Stat questions should read the repeated pattern behind the number.',
    ),
    _conceptMcqSeed(
      'forced_error_pressure',
      2,
      _QuizCategory.fun,
      '상대가 강한 공이나 깊은 코스로 압박해 만들어 낸 실수를 구분하는 기록 관점은?',
      'What stat lens separates errors caused by opponent pressure from self-made misses?',
      '포스드 에러 구분',
      'forced-error distinction',
      '포스드 에러를 구분하면 상대 압박이 만든 성과와 자신의 불필요한 실수를 다르게 평가할 수 있습니다.',
      'Distinguishing forced errors separates opponent-created pressure from avoidable misses.',
      '기록 해석은 누가 실수를 만들었는지까지 물어야 합니다.',
      'Stat interpretation should ask who created the error.',
    ),
    _conceptMcqSeed(
      'string_tension_control',
      2,
      _QuizCategory.technique,
      '공 컨트롤과 반발감을 조절하기 위해 선수 성향에 맞춰 스트링 강도를 관리하는 장비 판단은?',
      'What equipment judgment adjusts string tightness to balance control and power for the player?',
      '스트링 텐션 관리',
      'string-tension control',
      '스트링 텐션은 마법 해결책이 아니라 선수 스윙, 공 감각, 코트 조건과 함께 맞춰야 합니다.',
      'String tension is not a magic fix; it must match swing, feel, and court conditions.',
      '장비 문항은 장비 변화가 기술 습관과 맞는지 확인하세요.',
      'Equipment questions should check whether the change fits technique habits.',
    ),
    _conceptMcqSeed(
      'serve_shoulder_care',
      2,
      _QuizCategory.training,
      '서브량이 많은 날 이후 어깨 가동성, 회전근개 피로, 회복 신호를 관리하는 루틴은?',
      'What routine manages shoulder mobility, cuff fatigue, and recovery signs after a high-serve day?',
      '서브 어깨 회복 관리',
      'serve shoulder recovery care',
      '서브는 반복 회전 부하가 커서 통증 신호와 다음날 가동성을 기록하는 습관이 중요합니다.',
      'Serving creates repeated rotational load, so tracking pain signs and next-day mobility matters.',
      '훈련 문항은 기술 반복 수와 회복 신호를 함께 연결하세요.',
      'Training questions should connect repetition volume with recovery signs.',
    ),
    _conceptMcqSeed(
      'hip_rotation_chain',
      2,
      _QuizCategory.technique,
      '그라운드스트로크에서 다리, 엉덩이, 몸통 회전이 팔 스윙으로 이어지는 힘 전달은?',
      'What power transfer links legs, hips, and trunk rotation into the arm swing on groundstrokes?',
      '힙 회전 체인',
      'hip-rotation chain',
      '강한 샷은 팔만 빠른 것이 아니라 하체와 몸통 회전이 순서대로 연결될 때 안정됩니다.',
      'A strong shot is stable when legs and trunk rotation sequence into the arm, not from arm speed alone.',
      '스윙 문항은 라켓보다 먼저 몸통과 하체 순서를 보세요.',
      'Swing questions should inspect trunk and lower-body order before the racket.',
    ),
    _conceptMcqSeed(
      'return_position_adjustment',
      2,
      _QuizCategory.positions,
      '서버의 속도와 코스 습관에 맞춰 리턴 위치를 앞뒤 또는 좌우로 조정하는 판단은?',
      'What judgment adjusts return position forward, back, or sideways based on server pace and targets?',
      '리턴 위치 조정',
      'return-position adjustment',
      '리턴 위치는 고정된 예절이 아니라 서버 성향과 자신의 반응 시간을 맞추는 전술입니다.',
      'Return position is tactical, matching server tendencies and your reaction time.',
      '리턴 문항은 한 발 위치가 시간과 각도를 어떻게 바꾸는지 보세요.',
      'Return questions should ask how one step changes time and angle.',
    ),
    _conceptMcqSeed(
      'net_rush_split_stop',
      2,
      _QuizCategory.technique,
      '서브 앤 발리나 어프로치 뒤 네트로 뛰다가 상대 임팩트 순간 멈춰 반응하는 동작은?',
      'What movement stops into a split as the opponent hits after serve-and-volley or approach?',
      '네트 러시 스플릿',
      'net-rush split stop',
      '네트로 무조건 계속 달리면 패스 방향에 늦으므로 상대 타격 순간 반응 자세가 필요합니다.',
      'Running through the hit makes passing shots harder to cover, so a split stop is needed.',
      '전진 문항은 얼마나 빨리 갔는지보다 언제 멈췄는지 보세요.',
      'Net-rush questions should inspect when the player stops, not only how fast they moved.',
    ),
    _conceptMcqSeed(
      'pattern_memory_chart',
      3,
      _QuizCategory.tactics,
      '상대가 중요한 포인트에서 자주 쓰는 서브 코스와 랠리 방향을 기록해 다음 선택을 예측하는 준비는?',
      'What preparation records favorite serve targets and rally directions on key points to predict the next choice?',
      '패턴 메모리 차트',
      'pattern memory chart',
      '패턴 기록은 감으로 맞히는 것이 아니라 반복 선택을 근거로 첫 반응을 빠르게 합니다.',
      'Pattern tracking speeds the first reaction from repeated choices, not guessing by feel.',
      '전술 준비는 상대 습관이 어느 점수에서 반복되는지까지 보세요.',
      'Tactical prep should ask at which scores opponent habits repeat.',
    ),
  ];
}
...
List<_FootballQuizQuestion> _buildBaseballQuizPool() {
  return <_FootballQuizQuestion>[
    ...
    ..._nonFootballHundredFloorQuestions(SportCatalog.baseballId),
...
List<_FootballQuizQuestion> _buildBasketballQuizPool() {
  return <_FootballQuizQuestion>[
    ...
    ..._nonFootballHundredFloorQuestions(SportCatalog.basketballId),
...
List<_FootballQuizQuestion> _buildTennisQuizPool() {
  return <_FootballQuizQuestion>[
    ...
    ..._nonFootballHundredFloorQuestions(SportCatalog.tennisId),
...
QuizQualityHarnessReport buildQuizQualityHarnessReport({
  Map<String, int> minimumQuestionsBySport = const <String, int>{
    ...
    SportCatalog.baseballId: 100,
    SportCatalog.basketballId: 100,
