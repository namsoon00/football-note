...
class _LogsScreenState extends State<LogsScreen> {
  ...
  void didChangeDependencies() {
    ...
    _feedbackOnly = widget.optionRepository
        ...
        false;
    _lessonOnly = widget.optionRepository
            .getValue<bool>(_sportScopedKey(_lessonOnlyFilterKey)) ??
        false;
  }
  ...
  List<TrainingEntry> _applyFilters(
    ...
    return entries.where((entry) {
      ...
      final haystack = [
        ...
        entry.nextGoal,
        entry.jumpRopeNote,
        if (entry.breakfastDone) 'breakfast ${entry.breakfastRiceBowls}',
        if (entry.lunchDone) 'lunch ${entry.lunchRiceBowls}',
        if (entry.dinnerDone) 'dinner ${entry.dinnerRiceBowls}',
        entry.notes,
        entry.goal,
        entry.feedback,
        parentFeedback,
  ...
  Future<void> _openFilterSheet(BuildContext context) async {
    ...
    final lessonOnlyValue = _lessonOnly;
    ...
    final result = await showModalBottomSheet<_LogFilters>(
      ...
      builder: (context) {
        ...
        var localLessonOnly = lessonOnlyValue;
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SwitchListTile(
                    value: localLessonOnly,
                    onChanged: (value) =>
                        setModalState(() => localLessonOnly = value),
                    title: Text(l10n.filterLessonOnly),
                  ),
  ...
  Future<bool> _confirmDelete(BuildContext context, TrainingEntry entry) async {
    if (_isParentMode) {
      AppFeedback.showMessage(
        context,
        text: AppLocalizations.of(context)!.parentReadOnlyLogsMessage,
      );
      return false;
    }
  ...
  Future<void> _openParentFeedback(TrainingEntry entry) async {
    HapticFeedback.selectionClick();
  ...
  Future<void> _persistFilters(_LogFilters filters) async {
    await Future.wait([
      ...
      widget.optionRepository.setValue(
        _sportScopedKey(_lessonOnlyFilterKey),
        filters.lessonOnly,
      ),
    ]);
  }
  ...
  Widget _buildEntryRow({
    ...
    required String deleteKeyPrefix,
    ...
    return Dismissible(
      key: ValueKey(
        '$deleteKeyPrefix-${entry.key ?? '${entry.date.millisecondsSinceEpoch}-${entry.type}-${entry.notes.hashCode}'}',
      ),
...
class _EntryCard extends StatelessWidget {
  ...
  final VoidCallback onOpenParentFeedback;
  ...
  const _EntryCard({
    ...
    required this.onEdit,
    required this.onOpenParentFeedback,
  });
...
class _ParentFeedbackCornerMark extends StatelessWidget {
  ...
  const _ParentFeedbackCornerMark({required this.onTap});
...
String _buildListFocusText(
  ...
  required MealCoachingService mealCoachingService,
}) {
  if (entry.opponentTeam.trim().isNotEmpty) {
    return entry.opponentTeam.trim();
  }
  final mealStatus = mealCoachingService.statusForEntry(entry);
  if (mealStatus.completedMeals > 0) {
    final breakfast = entry.breakfastDone
        ? l10n.mealCompactSummary(l10n.mealBreakfast, entry.breakfastRiceBowls)
        : l10n.mealCompactSkipped(l10n.mealBreakfast);
    final lunch = entry.lunchDone
        ? l10n.mealCompactSummary(l10n.mealLunch, entry.lunchRiceBowls)
        : l10n.mealCompactSkipped(l10n.mealLunch);
    final dinner = entry.dinnerDone
        ? l10n.mealCompactSummary(l10n.mealDinner, entry.dinnerRiceBowls)
        : l10n.mealCompactSkipped(l10n.mealDinner);
