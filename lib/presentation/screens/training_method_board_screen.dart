...
class _TrainingMethodBoardScreenState extends State<TrainingMethodBoardScreen>
  ...
  Future<bool> _confirmReset(bool isKo) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(isKo ? '훈련스케치 초기화' : 'Reset training sketch'),
        content: Text(
          isKo
              ? '현재 보드를 정말 초기화할까요?'
  ...
  Future<bool> _saveBoard(bool isKo, {bool showFeedback = true}) async {
    ...
    if (showFeedback) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isKo ? '훈련스케치를 저장했습니다.' : 'Training sketch saved.'),
        ),
      );
    }
    return true;
  ...
  String _resolvedCurrentBoardTitle(bool isKo) {
    ...
    return isKo ? '훈련스케치' : 'Training Sketch';
  }
  ...
  Widget build(BuildContext context) {
    final isKo = Localizations.localeOf(context).languageCode == 'ko';
    final currentBoardTitle = _resolvedCurrentBoardTitle(isKo);
    return PopScope(
      canPop: false,
        ...
        appBar: AppBar(
          ...
          actions: [
            if (widget.presets.isNotEmpty)
              IconButton(
                tooltip: isKo ? '훈련스케치 복사' : 'Copy sketch',
