...
class _SpaceSpeedGameScreenState extends State<SpaceSpeedGameScreen> {
  ...
  SpaceSpeedPassState get _passState => SpaceSpeedPassState(
        attackerAIsPasser: _attackerAIsPasser,
        ballPhase: _ballSettling
            ? SpaceSpeedBallPhase.settling
            : (_ballFlying
                ? SpaceSpeedBallPhase.flying
                : SpaceSpeedBallPhase.held),
  ...
  void _updateBall() {
    ...
    _applyReceiverMagnet(currentDistance);
    _trackNearMiss();
    ...
    final reachedTarget =
        _distance(_ballX, _ballY, _targetX, _targetY) <= 0.032;
  ...
  void _applyReceiverMagnet(double currentDistance) {
    if (_goalChanceActive || !_ballFlying || _ballSettling) return;
    if (currentDistance > 0.12) return;
    final toX = _activeReceiverX - _ballX;
    final toY = _activeReceiverY - _ballY;
    final dist = math.max(0.001, math.sqrt((toX * toX) + (toY * toY)));
    final pull = ((0.12 - currentDistance) / 0.12).clamp(0.0, 1.0);
    final strength = (0.05 + (pull * 0.13)).clamp(0.05, 0.18);
    _ballVx += (toX / dist) * strength;
    _ballVy += (toY / dist) * strength;
    final speed = math.sqrt((_ballVx * _ballVx) + (_ballVy * _ballVy));
    if (speed > 1.28) {
      final scale = 1.28 / speed;
      _ballVx *= scale;
      _ballVy *= scale;
    }
  }
  ...
  _ReceivingWindowEval _receivingWindowEvaluation() {
    ...
    final forwardReach = (0.10 +
            (runnerNorm * 0.025) +
            (speed * anticipationTime * 0.24) +
            (_forwardWindow ? 0.015 : 0.0))
        .clamp(0.10, 0.19);
    final backReach = (0.010 + ((1 - speedNorm) * 0.008)).clamp(0.008, 0.018);
    final lateralReach = (0.078 +
            (runnerNorm * 0.024) +
            (_forwardWindow ? 0.012 : 0.0) -
            (speedNorm * 0.009))
        .clamp(0.065, 0.115);
    final eventScale = _eventReceiveWindowScale;
    final adjustedForwardReach = (forwardReach * eventScale).clamp(0.08, 0.23);
    final adjustedBackReach = (backReach * eventScale).clamp(0.007, 0.028);
    final adjustedLateralReach = (lateralReach * eventScale).clamp(0.055, 0.14);
